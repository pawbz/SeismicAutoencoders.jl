...
# ╔═╡ 3a25fc01-b44b-4f50-83be-83b8eb2386f5
using Clustering  # For K-means initialization
...
# ╔═╡ 10c71ff4-f9b4-4f19-9d2c-b0a4cd51d9ff
# Index-only search (non-differentiable) for shared codebook.
...
# ╔═╡ 8df6ef3f-8381-45d8-906e-caa37cc1622c
   """
...
# ╔═╡ af76063e-000e-43f0-bf39-493e8dd73d7e
# (Removed per-slot quantize_map — using shared-codebook `quantize_map_shared` instead)

# ╔═╡ e02992a7-3f4f-4247-91f3-f2c3e241762f
# GPU-safe EMA update and dead-entry reset for VectorQuantizerEMA
# (Removed EMA update machinery; using non-EMA VectorQuantizer only)
...
begin
    struct VQVAE{E,P,VQ,D}
        ...
        quantizer::VQ      # VectorQuantizer (shared, non-EMA)
    ...
    Flux.@layer VQVAE trainable = (encoder, pre_vq, decoder, quantizer)
    ...
    - `training`: whether to update the codebook (if applicable)
    ...
    function (m::VQVAE)(x; beta_commit::Float32=0.25f0, training::Bool=true)
        ...
        # Batch decode all T component vectors in one decoder call (GPU-friendly)
        z_q = vq_result.z_q  # (d, T, N_total)
        z_q_4 = reshape(z_q, m.d, m.T, n2, n3)          # (d, T, ntau, batch)
        z_q_4 = permutedims(z_q_4, (1, 3, 4, 2))        # (d, ntau, batch, T)
        dec_input_all = reshape(z_q_4, m.d, n2, n3 * m.T)  # (d, ntau, batch*T)

        # Single decoder call for all components: (nt, ntau, batch*T)
        xhat_all = m.decoder(dec_input_all)

        # Reshape per-component decoder outputs
        xhat_components = reshape(xhat_all, n1, n2, n3, m.T)   # (nt, ntau, batch, T)

        # Sum component results back into (nt, ntau, batch)
        xhat = mean(xhat_components, dims=4)[:, :, :, 1]

        # Reshape indices: (T, N_total) → (T, ntau, batch) for interpretability
        ...
        return (;
            ...
            embedding_loss = vq_result.embedding_loss,
    ...
    """
    get_codebook(m) = cpu(m.quantizer.embedding.weight)

    """
    Get cluster assignment histogram for a batch.
    Returns (K,) vector of percentages.
    """
    function get_cluster_percentages(m, x)
        result = encode(m, x)
        if m.T == 1
    ...
    """
    function filter_cluster(m, x, k::Int)
        result = encode(m, x)
        if m.T == 1
...
function loss_vqvae(model::VQVAE, x, para::VQVAE_Para; training::Bool=true)
    ...
    embedding_loss = result.embedding_loss
    entropy_loss = result.entropy_loss

    total = recon_loss + embedding_loss + para.entropy_weight * entropy_loss

    return (; total, recon_loss, embedding_loss, commit_loss=result.commit_loss, entropy_loss=entropy_loss, 
...
# ╔═╡ b6a4c2e6-8b84-4c7f-94b5-8d7ae7586fa8
begin
    # Non-EMA PyTorch-style VectorQuantizer (shared codebook across slots)
    mutable struct VectorQuantizer
        K::Int
        d::Int
        T::Int
        embedding::Flux.Embedding   # GPU-resident Flux.Embedding
        dead_count::Vector{Int}     # Track unused codes
        dead_threshold::Int         # Threshold for dead code reset
    end
    Flux.@layer VectorQuantizer trainable = (embedding,)

    function VectorQuantizer(K::Int, d::Int, T::Int; init_scale::Float32=1f0/Float32(K), dead_threshold::Int=2)
        emb_layer = Flux.Embedding(K => d, init=Flux.randn32)
        emb_layer = xpu(emb_layer)
        dead_count = zeros(Int, K)
        return VectorQuantizer(K, d, T, emb_layer, dead_count, dead_threshold)
    end
    ...
    function (vq::VectorQuantizer)(z_e::AbstractArray{Float32,3}; beta_commit::Float32=0.25f0, training::Bool=true)
        ...
        @assert T == vq.T "Expected T=$(vq.T), got $T"

        # Index search (non-differentiable)
        ...
        # Flatten encodings for perplexity: (K, T*N)
        ...
        Zygote.@ignore begin
        # Dead-code reset (training only): track usage per codebook entry and refresh rarely used codes
        if training
            counts = sum(enc_flat; dims=2)[:]  # dispatched to CPU-safe vector
            dead_mask = counts .< 0.5f0
            vq.dead_count .= ifelse.(dead_mask, vq.dead_count .+ 1, 0)
            reset_mask = vq.dead_count .>= vq.dead_threshold
            reset_idxs = findall(reset_mask)
            if !isempty(reset_idxs)
                d_flat = reshape(z_e, vq.d, vq.T * N)
                donor_idxs = rand(1:size(d_flat, 2), length(reset_idxs))
                donor_cols = d_flat[:, donor_idxs] .+ xpu(randn(Float32, vq.d, length(reset_idxs)) .* 0.01f0)
                emb_cpu = cpu(vq.embedding.weight)
                donor_cpu = cpu(donor_cols)
                for (i, k) in enumerate(reset_idxs)
                    emb_cpu[:, k] .= donor_cpu[:, i]
                end
                vq.embedding.weight .= xpu(emb_cpu)
                vq.dead_count[reset_idxs] .= 0
            end
        end
    end

        entropy_loss = sum(avg_probs_clamped .* log.(avg_probs_clamped))
        ...
        return (; z_q, codebook_loss, commit_loss, entropy_loss, embedding_loss, indices, perplexity, code_subscriptions)
    end

    function (vq::VectorQuantizer)(z_e::AbstractArray{Float32,2}; beta_commit::Float32=0.25f0, training::Bool=true)
        d_mat, M = size(z_e)
        @assert d_mat == vq.d "Expected embedding dim $(vq.d), got $d_mat"
        @assert M % vq.T == 0 "Second dimension $M not divisible by T=$(vq.T)"
        N = div(M, vq.T)
        z_e3 = reshape(z_e, d_mat, vq.T, N)
        return vq(z_e3; beta_commit=beta_commit, training=training)
    end

    function quantize_inference(vq::VectorQuantizer, z_e::AbstractMatrix{Float32})
        return vq(z_e; training=false)
    end
end
...
function get_vqvae(para::VQVAE_Para)
    ...
    # PyTorch-style non-EMA Vector Quantizer (shared codebook)
    quantizer = VectorQuantizer(para.K, para.d, para.T)

    # Decoder: d → nt waveform (independent decoding for each codebook vector)
    decoder, _, _ = get_vq_conv_decoder(para.nt, para.d;
...
# ╔═╡ d3d0fd95-b2d3-44d1-a576-9417e99b4cc9
# K-means codebook initialization for VQVAE
function initialize_codebook_kmeans!(model, data; batch_size=512, maxiter=100)
    # data: (nt, ntr) matrix of all waveforms
    # model: VQVAE instance
    println("[K-means init] Collecting encoder outputs...")
    x3 = add_dim3_reshape(data)
    n1, n2, n3 = size(x3)
    feat = model.encoder(x3)  # (flat_len, n2, n3)
    feat_flat = reshape(feat, size(feat, 1), :)
    z_pre = model.pre_vq(feat_flat)  # (d*T, N)
    N_total = size(z_pre, 2)
    d, T = model.d, model.T
    z_e = reshape(z_pre, d, T, N_total)  # (d, T, N_total)

    for t in 1:T
        println("[K-means init] Slot $t: running kmeans...")
        z_e_t = cpu(z_e[:, t, :])  # (d, N_total)
        # K-means expects (features, samples)
        result = kmeans(z_e_t, model.quantizer.K; maxiter=maxiter, display=:none)
        # Set codebook embedding for this slot
        # If using a shared codebook, just set all columns (no slot separation)
        # If using separate codebooks per slot, set accordingly
        # Here: shared codebook, so set all columns
        model.quantizer.embedding.weight .= xpu(Float32.(result.centers))
        println("[K-means init] Slot $t: codebook set.")
    end
    println("[K-means init] Done.")
end
...
function plot_training_dashboard(loss_history; title="VQ-VAE Training")
    ...
    traces = [
        PlutoPlotly.scatter(x=epochs, y=loss_history.train_recon, mode="lines", name="Train Recon", yaxis="y1", line=attr(color="#1f77b4")),
        PlutoPlotly.scatter(x=epochs, y=loss_history.test_recon, mode="lines", name="Test Recon", yaxis="y1", line=attr(color="#ff7f0e")),

        PlutoPlotly.scatter(x=epochs, y=loss_history.train_perplexity, mode="lines", name="Train Perplexity", yaxis="y2", line=attr(color="#2ca02c")),
        PlutoPlotly.scatter(x=epochs, y=loss_history.test_perplexity, mode="lines", name="Test Perplexity", yaxis="y2", line=attr(color="#d62728")),

        PlutoPlotly.scatter(x=epochs, y=loss_history.train_commit, mode="lines", name="Train Commit", yaxis="y3", line=attr(color="#9467bd")),
        PlutoPlotly.scatter(x=epochs, y=loss_history.test_commit, mode="lines", name="Test Commit", yaxis="y3", line=attr(color="#8c564b")),
    ]
    ...
    layout = Layout(
        ...
        xaxis=attr(title="Epoch", anchor="y3", showgrid=true, gridcolor=grid_color),

        yaxis=attr(title="Recon loss", domain=[0.70, 1.00], type="log", showgrid=true, gridcolor=grid_color, titlefont=attr(color="#1f77b4")),
        yaxis2=attr(title="Perplexity", domain=[0.36, 0.64], showgrid=true, gridcolor=grid_color, rangemode="tozero", titlefont=attr(color="#2ca02c")),
        yaxis3=attr(title="Commit loss", domain=[0.00, 0.30], type="log", showgrid=true, gridcolor=grid_color, titlefont=attr(color="#d62728")),

        legend=attr(x=1.02, xanchor="left", y=0.5, orientation="v", font=merge(font_spec, attr(size=12))),
        margin=attr(r=160),
    )
...
# ╔═╡ 4922ef4e-a04a-4e20-a527-ce054f5310ae
"""
...
manifest_format = "2.0"
project_hash = "c716a28b1c749c42bc4ebc5797ca146342d5ac1c"
