...
CPU-only notebook for saved v10 source-state artifacts. It reads
...
@bind ui_codebook_ncc_threshold Slider(0.50:0.05:0.99; default=0.80, show_value=true)

# ╔═╡ c300000a-0000-0000-0000-000000000001
@bind ui_mix_ntrials confirm(NumberField(10:10:1000; default=1000))
...
let
    ...
    for item in items
        waves, labels = if mode == "joint (K1×K2)" && !isempty(item.codebook_joint_waves)
...
if isnothing(selected_source_state_item)
    ...
    let item = selected_source_state_item
        ...
        layout = PlutoPlotly.Layout(
            title="All Codebook Waveforms — $(item.pair_label) seed=$(item.seed) (blue=decoder1, red=decoder2)",
            xaxis_title="Time (s)",
...
if isnothing(selected_source_state_item)
    ...
    let item = selected_source_state_item
        ...
        distance_label = isnothing(item.distance) ? "" : " $(round(Int, item.distance))km"
        layout = PlutoPlotly.Layout(
            title="Per-Stage Marginal Averaged Waveforms — $(item.pair_label) seed=$(item.seed)$distance_label (blue=decoder1/Stage1, red=decoder2/Stage2)",
...
mft_filter_bank = let
    npts_raw = begin
        ...
        isempty(items) ? begin
            items2 = [item for item in run_source_state_averages
                      if !isempty(item.codebook_stage2_waves)]
            isempty(items2) ? 0 : size(first(items2).codebook_stage2_waves, 1)
        end : size(first(items).codebook_stage1_waves, 1)
    end
    npts_raw == 0 ? nothing :
        mft.MFTFilterBank(dt, npts_raw, mft_periods;
...
global_average_mft_analyses = let
    ...
    @progress name="Global avg MFT" for pair_label in pair_labels
        ...
        dist = Float64(ref_item.distance)
        if !isnothing(mft_filter_bank)
            analyses[pair_label] = mft.analyze_causal_acausal_branches(
                global_c[1:n], global_ac[1:n], dist, mft_filter_bank;
                max_modes=mft_max_modes,
            )
        else
            c_trace  = mft.SeismicTrace(data=global_c[1:n],  dt=dt, distance=dist)
            ac_trace = mft.SeismicTrace(data=global_ac[1:n], dt=dt, distance=dist)
            analyses[pair_label] = mft.analyze_causal_acausal_branches(
...
_pair_mft_analyses_joint = let
    analyses = Dict{String,Any}()
    @progress name="Joint MFT" for pair_label in pair_labels
        items = [item for item in run_source_state_averages if item.pair_label == pair_label]
        cols_c  = Vector{Float64}[]
        cols_ac = Vector{Float64}[]
        dists   = Float64[]
        labels  = String[]
        for item in items
            ...
            for i in 1:nstates
                ...
                push!(dists, Float64(item.distance))
                label = i <= length(item.combo_labels) ? item.combo_labels[i] : string(i)
                push!(labels, "$(pair_label) seed $(item.seed) | $(label)")
            end
        end
        isempty(cols_c) && continue
        W_c  = reduce(hcat, cols_c)
        W_ac = reduce(hcat, cols_ac)
        if !isnothing(mft_filter_bank)
            analyses[pair_label] = mft.analyze_causal_acausal_branches(
                W_c, W_ac, dists, mft_filter_bank;
                state_labels=labels,
                max_modes=mft_max_modes,
            )
        else
            c_traces  = [mft.SeismicTrace(data=cols_c[i],  dt=dt, distance=dists[i]) for i in eachindex(cols_c)]
            ac_traces = [mft.SeismicTrace(data=cols_ac[i], dt=dt, distance=dists[i]) for i in eachindex(cols_ac)]
            analyses[pair_label] = mft.analyze_causal_acausal_branches(
...
_pair_mft_analyses_marginal = let
    analyses = Dict{String,Any}()
    @progress name="Marginal MFT" for pair_label in pair_labels
        items = [item for item in run_source_state_averages if item.pair_label == pair_label]
        cols_c  = Vector{Float64}[]
        cols_ac = Vector{Float64}[]
        dists   = Float64[]
        ...
        for item in items
            ...
            for k in 1:K1
                push!(cols_c,  Float64.(vec(item.marginal_stage1_c[:, k])))
                push!(cols_ac, Float64.(vec(item.marginal_stage1_ac[:, k])))
                push!(dists, Float64(item.distance))
                lbl = k <= length(item.marginal_stage1_labels) ? item.marginal_stage1_labels[k] : "s1=$k"
                push!(labels, "$(pair_label) seed $(item.seed) | S1 $lbl")
            end
            for k in 1:K2
                push!(cols_c,  Float64.(vec(item.marginal_stage2_c[:, k])))
                push!(cols_ac, Float64.(vec(item.marginal_stage2_ac[:, k])))
                push!(dists, Float64(item.distance))
                lbl = k <= length(item.marginal_stage2_labels) ? item.marginal_stage2_labels[k] : "s2=$k"
                push!(labels, "$(pair_label) seed $(item.seed) | S2 $lbl")
            end
        end
        isempty(cols_c) && continue
        W_c  = reduce(hcat, cols_c)
...
WideCell(mft.plot_branch_correlation(
    pair_mft_analyses[selected_plot_pair];
    title="MFT Branch Correlation — $(ui_mft_mode) $(selected_plot_pair)",
    reference_results=haskey(global_average_mft_analyses, selected_plot_pair) ? [global_average_mft_analyses[selected_plot_pair]] : mft.BranchAnalysisResult[],
...
if isempty(pair_labels) || !(selected_plot_pair in keys(pair_mft_analyses))
    ...
    WideCell(mft.plot_consensus_groupvelocity_picks(
        ...
        correlation_threshold=0.0,
        velocity_tolerance_fraction=0.1,
        title="Trained VQ-VAE $(ui_mft_mode) consensus $(selected_plot_pair)",
    ))
end
...
mft_analysis_test = let
    W_c  = Float64.(selected_source_state_item.causal)
    W_ac = Float64.(selected_source_state_item.acausal)
    dist = Float64(selected_source_state_item.distance)
    nstates = size(W_c, 2)
    labels = ["seed $i" for i in 1:nstates]
    if !isnothing(mft_filter_bank)
        mft.analyze_causal_acausal_branches(W_c, W_ac, dist, mft_filter_bank;
            state_labels=labels, max_modes=6)
    else
        c_traces  = [mft.SeismicTrace(data=W_c[:, i],  dt=dt, distance=dist) for i in 1:nstates]
        ac_traces = [mft.SeismicTrace(data=W_ac[:, i], dt=dt, distance=dist) for i in 1:nstates]
        mft.analyze_causal_acausal_branches(c_traces, ac_traces, mft_periods;
            state_labels=labels, max_modes=6,
            velocity_range=velocity_range,
            bandwidth_factor=bandwidth_factor,
            zero_pad_factor=zero_pad_factor)
...
codebook_mft_analysis = let
    ...
    if !isnothing(mft_filter_bank)
        reference_result = mft.analyze_causal_acausal_branches(
            global_c[1:n_ref], global_ac[1:n_ref], dist, mft_filter_bank;
            max_modes=mft_max_modes,
        )
        batch_result = mft.analyze_causal_acausal_branches(
            W_codes, W_codes, dist, mft_filter_bank;
        ...
        traces = [mft.SeismicTrace(data=W_codes[:, k], dt=dt, distance=dist)
...
best_mix_results = let
    items = [item for item in run_source_state_averages
             if item.pair_label == selected_plot_pair]
    isempty(items) && error("No runs for $selected_plot_pair.")
    !haskey(pair_mft_analyses, selected_plot_pair) && error("No MFT analysis for $selected_plot_pair.")

    mode = @isdefined(ui_codebook_mode) ? String(ui_codebook_mode) : "joint (K1×K2)"
    n_trials = @isdefined(ui_mix_ntrials) ? Int(ui_mix_ntrials) : 200
    top_K = 3
    vtol = 0.10

    # Extract reference (period, velocity) cloud from pair_mft_analyses
    ref_result = pair_mft_analyses[selected_plot_pair]
    ref_pv = Tuple{Float64,Float64}[]
    for st in ref_result.state_results
        hi = mft.high_correlation_indices(st; correlation_threshold=0.0)
        for ip in hi
            vavg = mft._matched_peak_average_velocities(
                st.causal_result.all_peaks[ip],
                st.acausal_result.all_peaks[ip],
                st.distance; velocity_tolerance_fraction=vtol)
            for v in vavg
                push!(ref_pv, (ref_result.periods[ip], v))
            end
        end
    end
    isempty(ref_pv) && error("Reference has no survived picks.")

    function score_from_result(res::Any, dist::Float64)
        cand_pv = Tuple{Float64,Float64}[]
        for ip in eachindex(mft_periods)
            for (t, _) in res.all_peaks[ip]
                isfinite(t) && t > 0.0 || continue
                v = dist / t
                isfinite(v) && v > 0.0 && push!(cand_pv, (mft_periods[ip], v))
            end
        end
        isempty(cand_pv) && return 0.0
        n_covered = sum(
            any(abs(p - rp) / max(rp, 1e-8) < 0.02 &&
                abs(v - rv) / max((abs(v) + abs(rv)) / 2, 1e-8) < vtol
                for (p, v) in cand_pv)
            for (rp, rv) in ref_pv)
        return n_covered / length(ref_pv)
    end

    # Pool all seeds into one joint codebook matrix
    all_cols       = Vector{Float64}[]
    all_col_labels = String[]
    ...
    for item in items
        ...
        for k in 1:K
            lbl = k <= length(labels) ? labels[k] : string(k)
            push!(all_cols, Float64.(waves[:, k]))
            push!(all_col_labels, "s$(item.seed)-$lbl")
        end
    end

    isempty(all_cols) && error("No codebook waveforms for $selected_plot_pair ($mode).")
    K_total = length(all_cols)
    W = reduce(hcat, all_cols)  # (nt × K_total)

    # Candidate weight vectors: basis vectors + random Dirichlet samples
    alphas = Vector{Float64}[]
    for k in 1:K_total
        α = zeros(K_total); α[k] = 1.0; push!(alphas, α)
    end
    for _ in 1:n_trials
        x = -log.(rand(K_total) .+ 1e-12)  # Dirichlet(1,...,1) via Gamma trick
        push!(alphas, x ./ sum(x))
    end

    # Materialise all candidate waveforms into one matrix, then batch-MFT in one shot
    W_cands = reduce(hcat, [W * α for α in alphas])   # (nt × n_cands)
    bank = isnothing(mft_filter_bank) ? nothing : mft_filter_bank
    mft_results = if !isnothing(bank)
        mft.perform_mft_analysis_batch!(bank, W_cands, dist; compute_phase=false)
    else
        # Fallback: serial per-trace MFT (no filter bank available)
        [mft.perform_mft_analysis(mft.SeismicTrace(data=W_cands[:, i], dt=dt, distance=dist),
             mft_periods; velocity_range=velocity_range,
             bandwidth_factor=bandwidth_factor, zero_pad_factor=zero_pad_factor)
         for i in 1:size(W_cands, 2)]
    end
    scored = [(; alpha=alphas[i], score=score_from_result(mft_results[i], dist),
                 mft_result=mft_results[i])
              for i in eachindex(alphas)]

    top = sort(scored, by=x -> -x.score)[1:min(top_K, length(scored))]

    # Stage 2: full analyze_causal_acausal_branches on the top-K mixed waveforms
    top_labels  = ["mix#$r score=$(round(top[r].score; digits=3))" for r in 1:length(top)]
    W_top       = reduce(hcat, [W * cand.alpha for cand in top])  # (nt × top_K)

    batch_result = if !isnothing(bank)
        mft.analyze_causal_acausal_branches(W_top, W_top, dist, bank;
            state_labels=top_labels, max_modes=mft_max_modes)
    else
        top_traces = [mft.SeismicTrace(data=W_top[:, r], dt=dt, distance=dist)
                      for r in 1:length(top)]
        mft.analyze_causal_acausal_branches(top_traces, top_traces, mft_periods;
            state_labels=top_labels, max_modes=mft_max_modes,
            velocity_range=velocity_range,
            bandwidth_factor=bandwidth_factor,
            zero_pad_factor=zero_pad_factor)
    ...
    [(; pair_label=selected_plot_pair, top, codebook_labels=all_col_labels, K=K_total,
        batch_result, consensus)]
end

# ╔═╡ c300000c-0000-0000-0000-000000000001
let
    if isempty(best_mix_results)
        md"No results."
    else
        sr = first(best_mix_results)
        ref_result = pair_mft_analyses[selected_plot_pair]
        mode = @isdefined(ui_codebook_mode) ? String(ui_codebook_mode) : "joint (K1×K2)"
        vtol = 0.10
        dist = Float64(first(ref_result.state_results).distance)

        traces = AbstractTrace[]

        # Reference picks — grey background
        first_ref_shown = false
        for st in ref_result.state_results
            hi = mft.high_correlation_indices(st; correlation_threshold=0.0)
            for ip in hi
                vavg = mft._matched_peak_average_velocities(
                    st.causal_result.all_peaks[ip], st.acausal_result.all_peaks[ip],
                    st.distance; velocity_tolerance_fraction=vtol)
                isempty(vavg) && continue
                push!(traces, PlutoPlotly.scatter(
                    x=fill(ref_result.periods[ip], length(vavg)), y=vavg,
                    mode="markers", name="Reference picks",
                    marker=PlutoPlotly.attr(color="lightgrey", size=5, symbol="circle"),
                    showlegend=!first_ref_shown, legendgroup="ref",
                ))
                first_ref_shown = true
            end
        end

        # Top-K candidate mixtures
        colors = ["royalblue", "tomato", "forestgreen"]
        for (rank, cand) in enumerate(sr.top)
            px = Float64[]; vy = Float64[]
            for ip in eachindex(mft_periods)
                for (t, _) in cand.mft_result.all_peaks[ip]
                    isfinite(t) && t > 0.0 || continue
                    v = dist / t
                    isfinite(v) && v > 0.0 && (push!(px, mft_periods[ip]); push!(vy, v))
                end
            end
            weight_str = join(
                ["$(sr.codebook_labels[k]):$(round(cand.alpha[k]; digits=2))"
                 for k in 1:sr.K if cand.alpha[k] > 0.05], ", ")
            push!(traces, PlutoPlotly.scatter(
                x=px, y=vy, mode="markers+lines",
                name="#$rank  score=$(round(cand.score; digits=3))  [$weight_str]",
                marker=PlutoPlotly.attr(color=get(colors, rank, "black"), size=7),
                line=PlutoPlotly.attr(color=get(colors, rank, "black"), width=1.5),
            ))
        end

        layout = PlutoPlotly.Layout(
            title="Top-$(length(sr.top)) Joint Mixture MFT Picks — $(sr.pair_label) ($mode)",
            xaxis_title="Period (s)", yaxis_title="Group velocity (km/s)",
            width=1000, height=520,
            plot_bgcolor="white", paper_bgcolor="white",
            legend=PlutoPlotly.attr(x=0.01, y=0.99, xanchor="left", yanchor="top", font_size=5),
        )
        WideCell(PlutoPlotly.plot(traces, layout))
    end
end

# ╔═╡ c300000d-0000-0000-0000-000000000001
let
    if isempty(best_mix_results)
        md"No results."
    else
        sr = first(best_mix_results)
        mode = @isdefined(ui_codebook_mode) ? String(ui_codebook_mode) : "joint (K1×K2)"
        p = mft.plot_consensus_groupvelocity_picks(
            sr.batch_result,
            sr.consensus;
            correlation_threshold=0.0,
            velocity_tolerance_fraction=0.10,
            title="Mixture Consensus Picks — $(sr.pair_label) ($mode)",
        )
        # Overlay global average causal / acausal reference picks
        if haskey(global_average_mft_analyses, sr.pair_label)
            ref = global_average_mft_analyses[sr.pair_label]
            ref_styles = [
                ("Causal",   ref.causal_result,   "#08306b", "circle-open"),
                ("Acausal",  ref.acausal_result,  "#7f0000", "diamond-open"),
            ]
            for (branch_name, mft_res, color, marker_symbol) in ref_styles
                max_peaks = maximum(length.(mft_res.all_peaks); init=0)
                for pidx in 1:max_peaks
                    xp = Float64[]; yp = Float64[]
                    for ip in eachindex(ref.periods)
                        peaks = mft_res.all_peaks[ip]
                        pidx > length(peaks) && continue
                        t, _ = peaks[pidx]
                        isfinite(t) && t > 0.0 || continue
                        v = ref.distance / t
                        isfinite(v) && v > 0.0 || continue
                        push!(xp, mft_res.periods[ip]); push!(yp, v)
                    end
                    isempty(xp) && continue
                    PlutoPlotly.addtraces!(p, PlutoPlotly.scatter(
                        x=xp, y=yp,
                        mode="lines+markers",
                        marker=PlutoPlotly.attr(size=9, color=color, symbol=marker_symbol,
                            line=PlutoPlotly.attr(color=color, width=1.4)),
                        line=PlutoPlotly.attr(color=color, width=2.6, dash="dash"),
                        name="Global avg $(lowercase(branch_name)) peak $pidx",
                    ))
                end
            end
        end
...
manifest_format = "2.0"
project_hash = "771a921f6c4952dd8c9be51367c321fcd56a01ad"
