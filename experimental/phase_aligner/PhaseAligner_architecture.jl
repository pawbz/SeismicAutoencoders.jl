...
# ╔═╡ 0c50729a-3a44-11f1-8319-759f35d77592
using CUDA,
    cuDNN,
    Flux,
    Zygote,
...
# ╔═╡ 0c50745e-3a44-11f1-8d7f-b1f39ab7fe4e
...
# ╔═╡ 0c50793e-3a44-11f1-91b9-e120aea844d0
xpu = gpu

# ╔═╡ 0c50797c-3a44-11f1-916c-010f4d9faf79
const activation_pa = x -> leakyrelu(x, 0.1f0)

# ╔═╡ 0c5079c2-3a44-11f1-aa7f-271ef1b71c3d
md"## Parameters"

# ╔═╡ 0c507a08-3a44-11f1-86c8-b38096a18cb9
Base.@kwdef struct PhaseAligner_Para
    ...
    gamma::Float32 = 0.001f0                 # regularization on φ
    ema_decay::Float32 = 0.99f0              # EMA decay for running phase baseline
    loss_norm::Symbol = :L1                  # :L1 (MAE) or :L2 (MSE)
    seed::Int = 42
end

# ╔═╡ 0c507bc0-3a44-11f1-ad1b-f36c0d941211
md"## Fourier Shift (reused from VQVAE-ST)"

# ╔═╡ 0c507bfc-3a44-11f1-9b3a-ef044757c111
begin
    """
...
# ╔═╡ 0c507f76-3a44-11f1-b91c-b909c40a04cd
md"## Phase Encoder (`φ`)"

# ╔═╡ 0c507fbc-3a44-11f1-a587-235e4c760cb5
begin
    """
...
# ╔═╡ 0c50850c-3a44-11f1-9373-bd8e37e41eb8
md"## EMA Phase Baseline"

# ╔═╡ 0c508548-3a44-11f1-bfa6-5dcb9930cacd
begin
    """
...
# ╔═╡ 0c50894e-3a44-11f1-b44d-61ece4f7ea46
md"## PhaseAligner Model"

# ╔═╡ 0c508980-3a44-11f1-98f0-f3ff547a64a6
begin
    """
...
# ╔═╡ 0c508dea-3a44-11f1-83d7-a9c37de64755
md"## Loss Functions"

# ╔═╡ 0c508e1c-3a44-11f1-b494-a5480a746cd9
begin
    """
    ...
    **Phase 2 loss — EMA baseline alignment**
    ...
    function alignment_loss(model::PhaseAligner, x::AbstractMatrix{Float32},
                             ...
                             training::Bool=true, loss_norm::Symbol=:L1)
        _norm = loss_norm === :L1 ? (a -> mean(abs, a)) : (a -> mean(abs2, a))
        phi = model(x)                          # (1, N)
        phi_B = baseline.phi_baseline            # scalar Float32

        recon  = _norm(phi .- phi_B)             # pull toward baseline
        reg    = gamma * _norm(phi)              # prevent drift
...
# ╔═╡ 0c5093bc-3a44-11f1-9b87-b5114ca23f98
md"## Training Loop"

# ╔═╡ 0c5093ee-3a44-11f1-accd-09af79a9cac2
Base.@kwdef struct PhaseAligner_Training_Para
    batchsize::Int             = 128
    nepoch_phase1::Int         = 100    # equivariance pre-training epochs
    nepoch_phase2::Int         = 300    # fine-tune + baseline epochs
    initial_lr_phase1::Float64 = 0.001
    initial_lr_phase2::Float64 = 0.001  # same as phase1: restarts reset momentum
    restart_period::Int        = 50     # cosine warm-restart period (epochs)
    nprint::Int                = 10
end

# ╔═╡ 0c509538-3a44-11f1-bede-ef9a25a4dfbe
begin
    ...
    function phase1_train_epoch!(model, opt_state, loader,
        ...
        for x in loader
            ...
            grads = Flux.gradient(model) do m
                equivariance_loss(m, xb, δ, gamma, max_shift; loss_norm=loss_norm)[1]
            end
    ...
    """One epoch of equivariance training + EMA baseline update (Phase 2)."""
    function phase2_train_epoch!(model, opt_state, loader,
        ...
        for x in loader
            ...
            Optimisers.update!(opt_state, model, grads[1])
            # Update baseline with batch mean (outside gradient)
            batch_phi = mean(cpu(model(xb)))
            update_baseline!(baseline, Float32(batch_phi))
        end
        GC.gc(false)
        CUDA.reclaim()
        return nothing
    ...
    function eval_equiv(model, x, gamma, max_shift; loss_norm::Symbol=:L1)
        xb = xpu(x)
        N  = size(xb, 2)
        δ  = (2f0 .* CUDA.rand(Float32, 1, N) .- 1f0) .* Float32(max_shift)
        total, equiv, reg = equivariance_loss(model, xb, δ, gamma, max_shift; loss_norm=loss_norm)
        return Float32(total), Float32(equiv), Float32(reg)
    end

    """Evaluate alignment loss on a batch (no gradient, no EMA update)."""
    function eval_align(model, x, baseline, gamma; loss_norm::Symbol=:L1)
        total, recon, reg = alignment_loss(model, xpu(x), baseline, gamma; training=false, loss_norm=loss_norm)
        return Float32(total), Float32(recon), Float32(reg)
    end
end

# ╔═╡ 0c509c74-3a44-11f1-bf6d-99b9b219bbeb
begin
    """
    ...
    function train_phase_aligner(model::PhaseAligner,
        ...
        # ── Phase 1: equivariance (cosine warm restarts) ───────────────────
        opt1 = Optimisers.setup(Optimisers.Adam(eta=training_para.initial_lr_phase1), model)
        T1   = training_para.restart_period
        @progress name = "Phase 1 (equivariance)" for epoch in 1:training_para.nepoch_phase1
            tr = eval_equiv(model, monitor_train, para.gamma, para.max_shift_samples; loss_norm=para.loss_norm)
            te = eval_equiv(model, monitor_test,  para.gamma, para.max_shift_samples; loss_norm=para.loss_norm)
            record_pa_metrics!(h, tr..., te...)
            phase1_train_epoch!(model, opt1, train_loader, para.gamma, para.max_shift_samples; loss_norm=para.loss_norm)
            log_pa_epoch(epoch, 1, tr[1], te[1], training_para.nprint)
            t_cos = mod1(epoch, T1)   # position within current restart cycle
            eta1  = training_para.initial_lr_phase1 * (1 + cos(π * t_cos / T1)) / 2
            Optimisers.adjust!(opt1; eta=eta1)
        end
        ...
        # ── Phase 2: fine-tune equivariance + update EMA baseline ──────────
        # NOTE: we keep the equivariance loss (NOT alignment_loss) so that
        # the encoder retains its learned φ spread. alignment_loss would
        # collapse all φ(x_i) to the same scalar and destroy Phase 1's result.
        # Fresh Adam state: second-moment estimates from Phase 1 are stale.
        opt2 = Optimisers.setup(Optimisers.Adam(eta=training_para.initial_lr_phase2), model)
        T2   = training_para.restart_period
        @progress name = "Phase 2 (fine-tune + baseline)" for epoch in 1:training_para.nepoch_phase2
            tr = eval_equiv(model, monitor_train, para.gamma, para.max_shift_samples; loss_norm=para.loss_norm)
            te = eval_equiv(model, monitor_test,  para.gamma, para.max_shift_samples; loss_norm=para.loss_norm)
            record_pa_metrics!(h, tr..., te...)
            phase2_train_epoch!(model, opt2, train_loader, baseline, para.gamma, para.max_shift_samples; loss_norm=para.loss_norm)
            log_pa_epoch(epoch, 2, tr[1], te[1], training_para.nprint)
            t_cos = mod1(epoch, T2)
            eta2  = training_para.initial_lr_phase2 * (1 + cos(π * t_cos / T2)) / 2
            Optimisers.adjust!(opt2; eta=eta2)
        end
...
# ╔═╡ 0c50a62c-3a44-11f1-9c1f-59cd80a6c6ec
md"## Model Factory"

# ╔═╡ 0c50a668-3a44-11f1-a705-7d8d13793036
"""
...
# ╔═╡ 0c50a884-3a44-11f1-961c-c7b7e0cf20b5
md"## Post-training Utilities"

# ╔═╡ 0c50a8b8-3a44-11f1-9a9c-4fe55aee013f
begin
    """
...
manifest_format = "2.0"
project_hash = "d4c50dcf5c5cb535a4c912915e903b68111e271f"
