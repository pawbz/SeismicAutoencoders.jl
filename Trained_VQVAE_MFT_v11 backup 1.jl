...
function _series_xy(rows, field::Symbol; error_cap=nothing)
    xs = Float64[]
    ys = Float64[]
    for row in rows
        ...
        if isfinite(x) && x > 0.0 && isfinite(y) && y > 0.0
            push!(xs, x)
            push!(ys, isnothing(error_cap) ? y : min(y, Float64(error_cap)))
        end
    end
    xs, ys
end
...
manifest_format = "2.0"
project_hash = "ca19004dbcd7715d44b84299a66e1984cd92c051"
