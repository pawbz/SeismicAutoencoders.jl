...
# ╔═╡ fd385b25-9588-4aaa-a04c-919510f5bf2c
using CUDA,
cuDNN,
    Enzyme,
    Flux,
...
# ╔═╡ 011f49f1-08de-4d2b-9cc5-d34a70e7b086
using PlutoHooks

# ╔═╡ 46f72d0d-0254-4192-8c0e-23949ac64937
using PlutoPlotly

# ╔═╡ bbb1e242-06fe-40eb-b7a9-b2c5d846776a
using PlutoLinks

# ╔═╡ 56495f31-6842-409d-a3d1-013b86ac0a18
using JLD2

# ╔═╡ 8fdb1dcf-e5df-4259-8d12-cd6fbadf4485
xpu = gpu

# ╔═╡ 0b711ebf-ec71-45ab-919f-c6a82f46b674
catvae = @ingredients("/mnt/NAS/EQData/SeismicAutoencoders/CatVAE_architecture.jl")

# ╔═╡ 0f67ca74-daf8-4510-ab5c-2ad54ea2eeee
D1 = jldopen("/mnt/NAS/Sanket_DRDO/ACSO_PKME.jld2")["data"];

# ╔═╡ 705fb696-ed6c-4d13-ac01-9700adfa7556
size(D1)

# ╔═╡ 0ff28417-3c06-4fea-b715-9d3dee26b479
data_real = xpu(Float32.(D1[257:512, :]))

# ╔═╡ bf9bf609-6948-4bda-bb91-69e423f8f8e8
plot(cpu(mean(data_real, dims=2)))

# ╔═╡ 8e877fee-b4e9-43af-af65-0ef794c0df41
plot(heatmap(z=data_real))

# ╔═╡ b48c20c4-9fa7-4245-b785-a8a634cc6fd0
# Generate synthetic seismograms with sinusoidally varying Ricker centers
function generate_synthetic_ricker(
    nt::Int=256;
    ntraces::Int=128,
    f0::Float64=10.0,         # dominant frequency (Hz)
    dt::Float64=0.004,        # sample interval (s)
    base_center::Int=128,     # base center sample
    amp::Int=20,              # modulation amplitude in samples
    period::Int=64,           # sinusoid period in traces
    noise_std::Float64=0.5,   # optional Gaussian noise
    normalize::Bool=true      # normalize each trace to unit max abs amplitude
)
    X = zeros(Float32, nt, ntraces)
    centers = Vector{Int}(undef, ntraces)
    α = (π * f0)^2

    @inbounds for i in 1:ntraces
        shift = round(Int, amp * sin(2π * (i - 1) / period))
        c = clamp(base_center + shift, 1, nt)
        centers[i] = c

        # time axis centered at c-th sample
        t = dt .* ((1:nt) .- c)
        trace = (1 .- 2 .* α .* t .^ 2) .* exp.(-α .* t .^ 2)

        if normalize
            m = maximum(abs, trace)
            m > 0 && (trace ./= m)
        end
        noise_std > 0 && (trace .+= noise_std .* randn(nt))
...
# ╔═╡ d8c6b212-f56d-423f-a7e4-ee33e007c432
data = hcat(xpu(generate_synthetic_ricker(period=100)[1]),  xpu(generate_synthetic_ricker(base_center=150, period=100)[1]))

# ╔═╡ 5be184eb-c7b0-4c54-b924-00ca52dc22cc
catvae_parameters = (; nt=256,
    p=10,
    k=4,
					 λ=1, 
    network_type=catvae.DenseAE(),
)

# ╔═╡ 145cd90c-024a-4b68-b371-3d86c7d46188
catvae_para = catvae.CatVAE_Para(; catvae_parameters...)

# ╔═╡ 59d7e73a-940c-4af6-b626-634d6ac60125
reload_network_button = @bind reload_network CounterButton("Reload Network")

# ╔═╡ 9f44ac19-9845-4d5b-85d4-0f484c478392
catvae_model, loss_history = @use_memo([reload_network]) do
    reload_network
    return catvae.get_catvae(catvae_para)
end

# ╔═╡ 27d9a222-5465-44d7-a7fa-a8780e14f042
I = cpu(catvae.classify(catvae_model, data_real))

# ╔═╡ 28065964-dbf6-4b2c-a769-6c19df94530a
plot(cpu(mean(data_real[:, findall(x->x==4, I)], dims=2)))

# ╔═╡ 68b4440b-7818-4e49-8e9e-4a83ce765a4b
plot(cpu(catvae.classify(catvae_model, data_real)))

# ╔═╡ 270616ac-6cee-4f0b-955d-f20f65de2565
plot(cpu(catvae_model(data).qcx))

# ╔═╡ 24e1528b-e852-464a-9af8-96372c355151
qcx = catvae_model(data).qcx

# ╔═╡ 1ce821c0-f356-41dc-b1a0-9f240dfc9a41
catvae_model(data)

# ╔═╡ e45db287-7e78-43c8-b345-005880fee41f
catvae_model.μ_prior

# ╔═╡ 6c7e0325-80bc-4f3d-92fe-c10ba34c8554
catvae.loss_catvae(catvae_model, data)

# ╔═╡ 8491cb0c-f275-45b9-918c-2b7a2fdea309
trained = @use_memo([]) do
    nepoch = 500
    training_para = catvae.Training_Para(nepoch=nepoch, initial_learning_rate=0.001)
    catvae.update(catvae_model, loss_history, data_real, data_real, training_para)
end

# ╔═╡ 21e6044c-030b-487a-b47a-ba945c6e293b
...
manifest_format = "2.0"
project_hash = "72e4cdd6053e130ad52ce6f3a837ec6a1373da87"
