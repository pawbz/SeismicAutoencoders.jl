...
set all inversion parameters below, and click **Run DSurfTomo**.

The notebook writes `DSurfTomo.in`, `surfdata.dat`, and `MOD` into a run
directory under `DSurfTomo_runs/`, then runs the inversion and plots the result.
"""
...
# ╔═╡ aa000023-0000-0000-0000-00000000002d
md"**Grid nz** (number of depth layers):"

# ╔═╡ aa000023-0000-0000-0000-00000000002e
@bind ui_nz NumberField(3:1:30; default=9)

# ╔═╡ aa000024-0000-0000-0000-000000000024
md"""
### Velocity bounds and inversion parameters
"""

# ╔═╡ aa000025-0000-0000-0000-000000000025
md"**Minimum Vs** (km/s):"

# ╔═╡ aa000025-0000-0000-0000-000000000026
@bind ui_minvel NumberField(0.1:0.1:5.0; default=1.5)

# ╔═╡ aa000025-0000-0000-0000-000000000027
md"**Maximum Vs** (km/s):"

# ╔═╡ aa000025-0000-0000-0000-000000000028
@bind ui_maxvel NumberField(0.5:0.1:10.0; default=5.0)

# ╔═╡ aa000025-0000-0000-0000-000000000029
md"**Weight** (data weight, ~1–10):"

# ╔═╡ aa000025-0000-0000-0000-00000000002a
@bind ui_weight NumberField(0.1:0.1:20.0; default=4.0)

# ╔═╡ aa000025-0000-0000-0000-00000000002b
md"**Damping** (regularization, ~0.5–2):"

# ╔═╡ aa000025-0000-0000-0000-00000000002c
@bind ui_damp NumberField(0.01:0.01:10.0; default=1.0)

# ╔═╡ aa000025-0000-0000-0000-00000000002d
md"**Sub-layers** for depth kernel (2–5):"

# ╔═╡ aa000025-0000-0000-0000-00000000002e
@bind ui_sublayers NumberField(2:1:5; default=3)

# ╔═╡ aa000025-0000-0000-0000-00000000002f
md"**Maximum iterations**:"

# ╔═╡ aa000025-0000-0000-0000-000000000030
@bind ui_maxiter NumberField(1:1:50; default=10)

# ╔═╡ aa000025-0000-0000-0000-000000000031
md"**Sparsity fraction** (0.02–0.5, lower = faster but less stable):"

# ╔═╡ aa000025-0000-0000-0000-000000000032
@bind ui_sparsity NumberField(0.01:0.01:1.0; default=0.2)

# ╔═╡ aa000025-0000-0000-0000-000000000033
md"**Noise level** (km/s, ~0.02):"

# ╔═╡ aa000025-0000-0000-0000-000000000034
@bind ui_noiselevel NumberField(0.001:0.001:0.5; default=0.02)

# ╔═╡ aa000025-0000-0000-0000-000000000035
md"**Outlier threshold** (km/s, data with residual > threshold are down-weighted):"

# ╔═╡ aa000025-0000-0000-0000-000000000036
@bind ui_threshold NumberField(0.1:0.1:10.0; default=3.0)

# ╔═╡ aa000026-0000-0000-0000-000000000026
md"""
### Depth layers

Comma-separated depth values in km. Should have at least **nz** values.
"""

# ╔═╡ aa000027-0000-0000-0000-000000000027
@bind ui_depth_layers TextField(60; default="0, 1, 2, 4, 8, 15, 25, 40, 60")
...
# ╔═╡ aa000030-0000-0000-0000-000000000030
md"""
### Wave type

Select the wave type recorded in your dispersion file:
"""

# ╔═╡ aa000030-0000-0000-0000-000000000031
@bind ui_wavetype Select(
    ["2 — Rayleigh group velocity", "1 — Rayleigh phase velocity",
     "3 — Love group velocity",     "4 — Love phase velocity"];
    default="2 — Rayleigh group velocity"
)

# ╔═╡ aa000031-0000-0000-0000-000000000031
...
begin
    ...
    function build_dsurftomo!(repo_dir)
        src_exe = joinpath(repo_dir, "src", "DSurfTomo")
        isfile(src_exe) && return src_exe
        assert_ok(run_command(`make`; cwd=joinpath(repo_dir, "src"));
                  context="Could not compile DSurfTomo")
        isfile(src_exe) || error("Build finished but executable not found: $src_exe")
        src_exe
    end
end
...
begin
    ensure_repo!(dsurftomo_root; update=false)
...
dsurftomo_result = let
    ...
    result = run_command(`$dsurftomo_exe DSurfTomo.in`; cwd=run_dir)
    assert_ok(result; context="DSurfTomo inversion failed")
    md"**DSurfTomo finished.** stdout length: $(length(result.stdout)) chars"
end
...
output_paths = let
    dsurftomo_result  # wait for inversion
    (;
...
@bind selected_depths confirm(
    MultiCheckBox(string.(depths); default=string.(depths[1:min(4,length(depths))])))
...
md"""
### GMT interpolation for depth plots
...
# ╔═╡ 7a51a97f-0cfa-4632-acf6-b1fef4f9b016
@bind gmt_interp_method Select(
    ["surface", "nearneighbor", "xyz2grd + grdfill"];
    default="surface")

# ╔═╡ 2c0dcbdd-9f4b-4a66-9335-2755e26208ef
@bind gmt_surface_tension Slider(0.0:0.05:1.0; default=0.35, show_value=true)

# ╔═╡ 1ade30ee-1eeb-4ca1-a471-6618d41f7e95
@bind gmt_search_radius_cells NumberField(1.0:0.5:20.0; default=4.0)

# ╔═╡ b2ec0279-e7e7-452d-80c2-6c52e124e9b4
@bind gmt_fill_remaining_holes CheckBox(default=true)

# ╔═╡ e248556e-1783-4f64-b6c1-4491c56d1e8e
@bind gmt_treat_bounds_as_missing CheckBox(default=true)

# ╔═╡ 55cd2725-73e6-4759-a19f-5f640ae46f0b
@bind gmt_bound_tolerance NumberField(0.0:0.0001:0.1; default=0.001)

# ╔═╡ d3d5857b-9832-4cb1-8898-039a60a226bc
md"""
### Map overlays
"""

# ╔═╡ 9554f0a5-bbe2-4829-b7c2-25ba58ee0d24
@bind gmt_show_stations CheckBox(default=true)

# ╔═╡ 86d2f80c-a8b5-4cfc-93f7-655c1e3e3872
@bind gmt_station_size_pt NumberField(4:1:18; default=9)

# ╔═╡ 1121e94e-3305-44b1-b6a7-18271d5bb09d
@bind gmt_show_remote_relief CheckBox(default=false)

# ╔═╡ d92fdbfb-8de3-44c5-9bc5-456f7045f18d
@bind gmt_relief_resolution Select(["01m", "30s", "15s"]; default="30s")

# ╔═╡ 76d23fe4-6a3d-4411-b94e-9a812c84bf54
@bind gmt_vs_transparency NumberField(0:5:80; default=35)
...
# ╔═╡ b1c88e12-c5dd-4e21-a4d4-35d9d489fb91
@bind display_depth Select(string.(depths); default=string(depths[1]))
