...
Short orchestration notebook. The SymVQVAE architecture owns the model,
...
# ╔═╡ 10000003-0000-0000-0000-000000000001
    ...
    vqvae = @ingredients("/mnt/NAS/EQData/SeismicAutoencoders/vqvae/SymVQVAE_architecture.jl")
...
# ╔═╡ 1000000c-0000-0000-0000-000000000005
Nmax = 25_000
...
begin
    if !@isdefined(compiled_model_cache_symvqvae)
        const compiled_model_cache_symvqvae = Ref{Any}(nothing)
    end
    if !@isdefined(compiled_model_key_cache_symvqvae)
        const compiled_model_key_cache_symvqvae = Ref{Any}(nothing)
    end
end
...
compiled_model = let
    ...
    elseif compiled_model_key_cache_symvqvae[] == selected_pairs_key && !isnothing(compiled_model_cache_symvqvae[])
        compiled_model_cache_symvqvae[]
    else
        ...
        compiled_model_key_cache_symvqvae[] = selected_pairs_key
        compiled_model_cache_symvqvae[]
    end
end
...
train_results = begin
    ...
    elseif isnothing(compiled_model)
        ...
        vqvae.train_selected_pairs_lazy(
            ...
            seeds=run_seeds,
            training_para=training_para,
            save_root=joinpath(data_filepath, "SavedModels", "symvqvae_K=$(string(cm.para.K))"),
...
symvqvae_convergence_summary = let results = train_results
    if isnothing(results)
        "symvqvae_training\nstatus=not_started"
    else
        io = IOBuffer()
        println(io, "symvqvae_training")
        for result in results
            ...
            println(io, "Nmax=$(Nmax)")
            println(io, "nepoch=$(result.training_para.nepoch)")
            println(io, "n_compiled_encoder=$(@isdefined(compiled_model) && !isnothing(compiled_model) ? compiled_model.n_compiled_encoder : missing)")
...
if isnothing(selected_result)
    ...
    WideCell(vqvae.plot_training_dashboard(selected_result.loss_history;
        title="SymVQVAE Training Dashboard ($(result_title_context(selected_result)))"))
end
