...
Base.@kwdef struct VQVAE_Para
    ...
    shift_temperature::Float32 = 1f0     # legacy (unused for continuous residual shift regression)
    shift_codebook_size::Int = 33        # legacy (unused for continuous residual shift regression)
...
begin
    struct ResidualShiftRegressorNet{C,H}
        chain::C
        head::H
        max_shift_samples::Float32
    end
    Flux.@layer ResidualShiftRegressorNet trainable = (chain, head)

    function (m::ResidualShiftRegressorNet)(residual_latent::AbstractMatrix{Float32}, condition=nothing)
        feat = m.chain(residual_latent)
        raw_shift = m.head(feat)  # (1, B)
        shifts = m.max_shift_samples .* tanh.(raw_shift)
        # Keep legacy fields for downstream compatibility with existing notebooks.
        return (; shifts, shift_raw=raw_shift,
            shift_logits=nothing, shift_probs=nothing,
            shift_hard=nothing, shift_index=nothing)
    end

    function build_residual_shift_regressor_net(d::Int, max_shift::Int;
        ...
        head = Dense(hidden, 1)
        return ResidualShiftRegressorNet(chain, head, Float32(max_shift))
    end
    ...
    function encode(m::VQVAE_V3, x; beta_commit::Float32=0.25f0, training::Bool=false, condition=nothing)
        ...
        L, _, B = size(feat_map)
        # Project each latent-time slot independently via 1x1 conv: (L, C, B) -> (L, d, B)
        slot_map = m.slot_projector(feat_map)
        # Materialize the permutation on GPU to avoid scalar-indexing fallbacks
        # when reductions (e.g., mean) are applied downstream.
        slot_latents = CUDA.permutedims(slot_map, (2, 1, 3))
...
function get_vqvae(para)
    ...
    # Per-slot linear projection implemented as a 1x1 conv for CUDA/Zygote stability.
    slot_projector = Conv((1,), enc_channels => para.d) |> xpu
    ...
    shift_net = build_residual_shift_regressor_net(para.d * latent_len, para.max_shift_samples;
