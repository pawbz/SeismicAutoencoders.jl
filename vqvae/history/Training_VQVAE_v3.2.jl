...
vqvae = @ingredients("/mnt/NAS/EQData/SeismicAutoencoders/VQVAE_architecture_v3.2.jl")
...
vqvae_parameters = (;
    nt=nth,
    d=24,            # codebook embedding dimension (matches SymAE p)
    K=2,             # codebook size (matches SymAE k)
    ...
    entropy_weight=0.0f0,  # stronger anti-collapse regularization
    ...
    velocity_range=(2, 6),
                    ...
                    per_slot_codebook=true,
					per_position=true,
					per_position_use_lstm=true,
                    per_position_lstm_hidden=16,
					# km/s; (vmin, vmax) group velocity range for window
    # epsilon=1e-1,
)
...
begin
    trained
    combo_labels = model.per_position ? cross.labels : vqvae.combination_labels(vqvae_para.K, vqvae_para.T)
...
let
    ...
    for combo_idx in 1:ncomb
        ...
        ks_tuple = state_tuple_for_model(combo_idx)
        _, sel_ac = vqvae.filter_cluster(model, data.D_ac_all, ks_tuple; cluster_filter_kwargs()...)
        _, sel_c = vqvae.filter_cluster(model, data.D_c_all, ks_tuple; cluster_filter_kwargs()...)
...
# ╔═╡ 8296542a-b465-49bf-bc73-fba39e224dc8
let
    probs = slot_entropy_pooled.probs  # (K, T)
    probs_sorted = similar(probs)
    for t in 1:size(probs, 2)
        probs_sorted[:, t] = sort(probs[:, t]; rev=true)
    end
    PlutoPlotly.plot([PlutoPlotly.heatmap(
        z=probs_sorted, x=1:size(probs,2), y=1:size(probs,1),
        colorscale="Viridis", colorbar=attr(title="sorted P(rank | slot)")
    )], Layout(title=attr(text="Sorted per-slot code usage")))
end

# ╔═╡ bab155f9-5e4a-4701-a48f-420609355a00
let
    trained
    probs = slot_entropy_pooled.probs            # (K, T): code x slot
    heat = PlutoPlotly.heatmap(
        z=probs,
        y=collect(1:size(probs, 1)),             # code index
        x=collect(1:size(probs, 2)),             # latent slot
        colorscale="Viridis",
        colorbar=attr(title="P(code | slot)")
    )
    layout = Layout(
        title=attr(text="Code usage histogram by latent slot $(selected_pair[1])-$(selected_pair[2]) $(round(Int, data_bundle.distance))km"),
        width=900,
        height=420,
        yaxis=attr(title="Code"),
        xaxis=attr(title="Latent slot"),
        plot_bgcolor="white",
        paper_bgcolor="white"
    )
    WideCell(PlutoPlotly.plot([heat], layout))
end

# ╔═╡ 4b7c3278-8417-4a65-b52a-d4e40557c7ff
plot(vec(mean(slot_entropy_pooled.probs, dims=2)))

# ╔═╡ 26350ac1-bba1-4a0e-9315-a461d6bbf45f
begin
	trained
	(typeof(model.quantizer), model.per_position, model.T)
end

# ╔═╡ 8a8d4a5d-4f0d-4f57-83a3-29ca9e26f6a2
inverse_occupancy_weighted_stack = let
    trained
    nstates = size(cluster_avg_ac, 2)
    eps_count = 1e-3
    beta_inv_occ = 0.6f0

    counts_ac = zeros(Float32, nstates)
    counts_c = zeros(Float32, nstates)
    for combo_idx in 1:nstates
        ks_tuple = model.per_position ? (combo_idx,) : Tuple(vqvae.combination_digits(combo_idx, vqvae_para.K, vqvae_para.T))
        kwargs = model.per_position ? (; time_index=selected_latent_pos) : (;)
        _, sel_ac = vqvae.filter_cluster(model, data.D_ac_all, ks_tuple; kwargs...)
        _, sel_c = vqvae.filter_cluster(model, data.D_c_all, ks_tuple; kwargs...)
        counts_ac[combo_idx] = Float32(length(sel_ac))
        counts_c[combo_idx] = Float32(length(sel_c))
    end

    counts_mean = (counts_ac .+ counts_c) ./ 2f0
    raw_w = 1f0 ./ ((counts_mean .+ eps_count) .^ beta_inv_occ)
    weights = raw_w ./ max(sum(raw_w), eps(Float32))

    function unit_rms(v)
        s = sqrt(mean(abs2, v)) + 1f-8
        v ./ s
    end

    ac_norm = hcat([unit_rms(vec(cluster_avg_ac[:, i])) for i in 1:nstates]...)
    c_norm = hcat([unit_rms(vec(cluster_avg_c[:, i])) for i in 1:nstates]...)

    weighted_ac = ac_norm * weights
    weighted_c = c_norm * weights
    weighted_joined = vcat(reverse(weighted_ac), weighted_c)

    (; counts_ac, counts_c, counts_mean, weights,
        beta_inv_occ, weighted_ac, weighted_c, weighted_joined)
end

# ╔═╡ 5d598f2b-7e65-4d3e-b167-0e1abf4120b6
let
    trained
    st = inverse_occupancy_weighted_stack
    t_neg = [-(nth - i + 1) * dt for i in 1:nth]
    t_pos = [i * dt for i in 1:nth]
    t_full = [t_neg; t_pos]

    tr_weighted = PlutoPlotly.scatter(
        x=t_full,
        y=st.weighted_joined,
        mode="lines",
        line=attr(color="firebrick", width=3),
        name="Inverse-occupancy weighted stack"
    )

    tr_global = PlutoPlotly.scatter(
        x=t_full,
        y=vcat(reverse(vec(mean(cpu(data.D_ac_all); dims=2))), vec(mean(cpu(data.D_c_all); dims=2))),
        mode="lines",
        line=attr(color="black", width=2, dash="dot"),
        name="Unweighted global stack"
    )

    layout = Layout(
        title=attr(text="Selected-slot weighted stack (slot=$(selected_latent_pos), β=$(round(st.beta_inv_occ; digits=2)))"),
        width=950,
        height=430,
        xaxis=attr(title="Lag (s)"),
        yaxis=attr(title="Normalized amplitude"),
        plot_bgcolor="white",
        paper_bgcolor="white",
        shapes=velocity_vlines(t_vmin, t_vmax; symmetric=true)
    )
    WideCell(PlutoPlotly.plot([tr_weighted, tr_global], layout))
end

# ╔═╡ 4b14c0db-9917-4452-a35e-741284d937c7
# ╠═╡ disabled = true
#=╠═╡
 cluster_avg_ac,  cluster_avg_c = let
    K = vqvae_para.K
    T = model.T

    c_by_t = Vector{Matrix{Float32}}(undef, T)
    ac_by_t = Vector{Matrix{Float32}}(undef, T)
    for t in 1:T
        c_by_t[t] = vqvae.get_cluster_averages(model, data.D_c_all; time_index=t)
        ac_by_t[t] = vqvae.get_cluster_averages(model, data.D_ac_all; time_index=t)
    end

    nt = size(c_by_t[1], 1)
    cluster_avg_c_corrpool = zeros(Float32, nt, K)
    cluster_avg_ac_corrpool = zeros(Float32, nt, K)
    selected_slots_per_k = [Int[] for _ in 1:K]
    nslots_used_per_k = zeros(Int, K)

    for k in 1:K
        for t in 1:T
            r = safe_corr(vec(c_by_t[t][:, k]), vec(ac_by_t[t][:, k]))
            if r > corr_pool_threshold
                cluster_avg_c_corrpool[:, k] .+= c_by_t[t][:, k]
                cluster_avg_ac_corrpool[:, k] .+= ac_by_t[t][:, k]
                nslots_used_per_k[k] += 1
                push!(selected_slots_per_k[k], t)
            end
        end

        if nslots_used_per_k[k] > 0
            cluster_avg_c_corrpool[:, k] ./= nslots_used_per_k[k]
            cluster_avg_ac_corrpool[:, k] ./= nslots_used_per_k[k]
        else
            # fallback: pick best slot for this k
            r_all = [safe_corr(vec(c_by_t[t][:, k]), vec(ac_by_t[t][:, k])) for t in 1:T]
            tbest = argmax(r_all)
            cluster_avg_c_corrpool[:, k] .= c_by_t[tbest][:, k]
            cluster_avg_ac_corrpool[:, k] .= ac_by_t[tbest][:, k]
            nslots_used_per_k[k] = 1
            push!(selected_slots_per_k[k], tbest)
        end
    end
	 cluster_avg_ac_corrpool,  cluster_avg_c_corrpool
end
  ╠═╡ =#
...
begin
	trained
	cluster_avg_kwargs = (model.per_position && @isdefined(selected_latent_pos)) ? (; time_index=selected_latent_pos) : (;)
	    cluster_avg_c = vqvae.get_cluster_averages(model, data.D_c_all; cluster_avg_kwargs...)
		cluster_avg_ac = vqvae.get_cluster_averages(model, data.D_ac_all; cluster_avg_kwargs...)
end
...
manifest_format = "2.0"
project_hash = "781a4f665ae2578b25aefbfcccc97d2fa7bd1a2c"
