...
training_para = let
    ...
    vqvae.VQVAE_Training_Para(
        nepoch=50,
        initial_learning_rate=0.0001,  # ↑ from 0.001,
        # stop_on_recon_loss = 0.98,
        lr_decay=0.99,
        pairs_per_batch=1,
        per_pair_batchsize=5000,
...
vqvae_parameters = (;
    ...
    # multiscale RVQ: coherent (per-pair self-attention pooling) + nuisance residuals
    ...
    ema_decay_large=0.99f0,
		...
		attn_temperature=1.0f0,
        attn_blend=0.1f0,
        attn_self_bias=0.0f0,
	    per_position=false,
