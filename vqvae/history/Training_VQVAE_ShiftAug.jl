...
# ╔═╡ 71d5d37c-3aef-11f1-b670-a50e32cc1004
begin
...
# ╔═╡ 5839dfb5-4b85-417f-aa48-1216052cd1e8
using Functors

# ╔═╡ 71d5d598-3aef-11f1-bc63-056c1212e89b
using PlutoLinks: @ingredients

# ╔═╡ 4f87ced9-1f9a-49e1-bf13-10f10bd8e152
using Enzyme

# ╔═╡ 426f10f7-d0ab-4ab5-a884-ae863e60b445
using Distances

# ╔═╡ 1d90dbff-9acc-4144-b254-151af6b17132
using MLUtils

# ╔═╡ c57a7878-5747-459b-b8b4-b63d1a5225fb
using BenchmarkTools

# ╔═╡ fe875d8a-d4fc-4b42-8699-f983518fc469
using ParameterSchedulers

# ╔═╡ 71d5d5e8-3aef-11f1-8f67-210eb327dcbe
TableOfContents(include_definitions=true)

# ╔═╡ 71d5d624-3aef-11f1-b7d5-1d946b8701fa
xpu = gpu

# ╔═╡ 71d5d660-3aef-11f1-bd72-6938bc63015b
...
The hope is that there exists an **alignment code** `k*` in the codebook such that
for *every* waveform `x_i` there is exactly one shift `τ_{i,j*}` that maps it to `k*`.
Those shifts collectively bring all waveforms into the same reference frame.

## Finding the canonical alignment via code assignment

After training, build the `(N × M)` code matrix `C[i,j]` = codebook entry for waveform `i` at shift `τ_j`.

The **alignment code** `k*` is the codebook entry for which the most waveforms have
*exactly one* shift that maps to it.  For each such waveform `x_i`:
- Find the unique `τ_{i,j*}` with `C[i,j*] = k*`
- Back-shift: `x_i_aligned = shift(x_i, −τ_{i,j*})`
- Average → coherent stack (no reconstruction loss used at all)

## Two-step workflow
```
Step 1:  Augment ✕M  →  Train VQ-VAE on augmented data
Step 2:  Code matrix  →  Find k*  →  Back-shift by –τ_{i,j*}  →  Stack
```
"""

# ╔═╡ 71d5d9c6-3aef-11f1-8741-0f75303776fd
md"## Load Architecture"

# ╔═╡ 71d5da0a-3aef-11f1-8a5b-7b2fd3e0f76a
vqvae = @ingredients("/mnt/NAS/EQData/SeismicAutoencoders/VQVAE_architecture_v3.1.jl")

# ╔═╡ 71d5da48-3aef-11f1-8e4e-1317b2357d70
md"## Fourier Shift Utility"

# ╔═╡ 71d5da84-3aef-11f1-8c12-4d4781d922f2
begin
    """
...
# ╔═╡ 71d5dd5e-3aef-11f1-bbb7-c9431a0398b6
md"## Shift Augmentation"

# ╔═╡ 71d5dd9a-3aef-11f1-ab83-2d4a468ac808
begin
    """
...
# ╔═╡ 71d5e36c-3aef-11f1-8ebf-83de9a5ae8d8
md"## Post-training Utilities"

# ╔═╡ 71d5e3b0-3aef-11f1-a939-852f0bd504dd
begin
    """
        get_codes(model, X; batchsize) -> codes (N,)

    Run `X` (nt, N) through the encoder and return the codebook index per waveform.
    """
    function get_codes(model, X::AbstractMatrix{Float32}; batchsize::Int=256)
        N = size(X, 2)
        codes  = zeros(Int, N)
        ...
        for xb in loader
            ...
            codes[offset+1:offset+nb] .= vec(result.codebook_indices[1, :])
            offset += nb
        end
        return codes
    end
...
# ╔═╡ 71d5ece0-3aef-11f1-a69c-d545c032e63c
md"---
...
# ╔═╡ 71d5ed88-3aef-11f1-8588-2b9e5c648911
begin
    nt_syn        = 300
    f0_syn        = 0.05f0
...
# ╔═╡ 71d5eee8-3aef-11f1-98b4-b99dc252d07b
begin
    """
...
# ╔═╡ 71d5f1ae-3aef-11f1-a14e-3168fc27fcf6
md"### Model & Augmentation Parameters (Synthetic)"

# ╔═╡ 71d5f1ea-3aef-11f1-a033-3336b15b2315
...
# ╔═╡ 71d5f3f2-3aef-11f1-a66a-37f491789a1e
...
# ╔═╡ 71d5f4b0-3aef-11f1-912e-0d7fd8cac42a
md"### Data Preview: Raw vs Augmented (Synthetic)"

# ╔═╡ b93021ac-3279-4ea4-acc5-02a98d8549ea
let
    snr0   = snr_levels[1]         # use first SNR for preview
    X_raw_prev, _ = make_ricker_syn(; snr=snr0)
    grid_prev = make_shift_grid(nt_syn)
    X_aug_prev, tau_aug_prev, orig_idx_prev, sg_prev =
        augment_with_shifts(X_raw_prev, M_aug_syn, max_shift_syn, grid_prev)
    X_aug_prev = normalise_traces(X_aug_prev)
    ts         = 1:nt_syn
    n_show     = 30                  # traces to display in each panel
    scale      = 2.5f0               # vertical spacing between traces

    # --- left panel: raw traces (n_show evenly spaced) ---
    idxs_raw = round.(Int, range(1, size(X_raw_prev, 2); length=n_show))
    traces_raw = [PlutoPlotly.scatter(
        x=ts, y=X_raw_prev[:, i] .+ (j-1)*scale,
        mode="lines", showlegend=false,
        line=attr(color="#1f77b4", width=0.8), opacity=0.7)
        for (j, i) in enumerate(idxs_raw)]

    # --- right panel: sample of augmented traces (pick orig_idx == 1, all M shifts) ---
    idxs_aug = findall(==(1), orig_idx_prev)   # all M shifts of waveform 1
    traces_aug = [PlutoPlotly.scatter(
        x=ts, y=X_aug_prev[:, idxs_aug[j]] .+ (j-1)*scale,
        mode="lines", showlegend=false,
        name="τ=$(round(tau_aug_prev[idxs_aug[j]]; digits=1))",
        line=attr(color="#d62728", width=0.8), opacity=0.7)
        for j in 1:length(idxs_aug)]

    p_raw = PlutoPlotly.plot(traces_raw, Layout(
        title=attr(text="Raw ($(size(X_raw_prev,2)) traces, SNR=$snr0)"),
        xaxis=attr(title="Sample"), yaxis=attr(showticklabels=false),
        height=420, width=440, plot_bgcolor="white", paper_bgcolor="white"))
    p_aug = PlutoPlotly.plot(traces_aug, Layout(
        title=attr(text="All M=$M_aug_syn shifts of 1 waveform (augmented)"),
        xaxis=attr(title="Sample"), yaxis=attr(showticklabels=false),
        height=420, width=440, plot_bgcolor="white", paper_bgcolor="white"))
    [p_raw p_aug]
end

# ╔═╡ 71d5f4ec-3aef-11f1-aae9-b11f258dd339
md"### Run Synthetic Tests at Multiple SNRs"

# ╔═╡ 71d5f956-3aef-11f1-a29e-3d8ecf5fe14b
@bind snr_sel Select(string.(snr_levels); default=string(snr_levels[1]))

# ╔═╡ 71d5f91a-3aef-11f1-9c9d-839c80223f47
md"### Waveforms per Cluster (Original vs Augmented)"

# ╔═╡ a02a93ec-2163-425f-b379-eb389c0086cc
md"### Cluster Averages (Augmented Data)"

# ╔═╡ 71d5fc62-3aef-11f1-afda-db0d1922f37b
md"### Coherent stack per SNR"

# ╔═╡ 71d5ffde-3aef-11f1-bce9-ddff0038a325
md"### Training Loss Curve"

# ╔═╡ 71d60270-3aef-11f1-a5a6-454de9fe610c
md"### Code Usage per SNR"

# ╔═╡ 71d602ae-3aef-11f1-9abf-f78c402a8e0e
md""  # removed (histogram above covers cluster counts)

# ╔═╡ 71d6046e-3aef-11f1-80a9-1fc6eae13c0d
md"---
## Real Data Section
### Load 410Ps Receiver Functions
"

# ╔═╡ 71d604d2-3aef-11f1-be02-b500bc1d203c
begin
...
# ╔═╡ 71d605d6-3aef-11f1-ac60-63c5afadf16f
begin
    s_sel   = "1"
...
# ╔═╡ 9bee5f23-4ba9-4cb3-b5f3-43aef5d30214
plot(true_rf[:, 10])

# ╔═╡ 71d60718-3aef-11f1-a615-9d1148aedf32
md"### Augmentation & Model Parameters (Real Data)"

# ╔═╡ 71d6075c-3aef-11f1-91c8-2dec106d1252
begin
    M_aug_real     = 20          # augmentation factor
    max_shift_real = 20          # ±20 samples (adjust to your data's expected lag)
end

# ╔═╡ 71d60810-3aef-11f1-87b0-5b768f5bc390
...
# ╔═╡ 71d609be-3aef-11f1-a03d-dd250604c00e
...
# ╔═╡ 71d60a7c-3aef-11f1-aae1-7361fbf7a467
reload_real_button = @bind reload_real CounterButton("Reload Real Model")

# ╔═╡ 71d60dce-3aef-11f1-b9ff-1d4c0bb008b7
md"### Real Data: Training Loss"

# ╔═╡ 3b065afe-3afb-11f1-8dde-939059fb46d0
begin
    """
        get_cluster_averages(model, X_aug, K; batchsize) 
            -> (cluster_means, cluster_counts, codes_aug)

    Run all augmented waveforms through the encoder and average within each cluster.
    Returns the empirical cluster centroid as seen by the VQ-VAE on the augmented data.

    - `X_aug`           : `(nt, N*M)` augmented waveforms (CPU, normalised)
    - `cluster_means`   : `(nt, K)` mean waveform per code
    - `cluster_counts`  : `(K,)` number of augmented waveforms per code
    - `codes_aug`       : `(N*M,)` code assignment for every augmented waveform
    """
    function get_cluster_averages(model, X_aug::AbstractMatrix{Float32}, K::Int,
                                   orig_idx::AbstractVector{Int};
                                   batchsize::Int=256)
        nt, N_aug = size(X_aug)
        N = maximum(orig_idx)          # number of original waveforms
        codes_aug = zeros(Int, N_aug)
        loader = Flux.DataLoader(X_aug; batchsize=batchsize, shuffle=false)
        offset = 0
        for xb in loader
            nb = size(xb, 2)
            result = vqvae.encode(model, xpu(xb); training=false)
            codes_aug[offset+1:offset+nb] .= vec(result.codebook_indices[1, :])
            offset += nb
        end
        cluster_means  = zeros(Float32, nt, K)
        cluster_counts = zeros(Int, K)
        for k in 1:K
            # Allow at most one shifted copy per original waveform (first match wins).
            # This prevents multi-voting: a single waveform shifted into the same
            # cluster multiple times would otherwise bias the cluster mean.
            seen = falses(N)
            for j in 1:N_aug
                if codes_aug[j] == k && !seen[orig_idx[j]]
                    cluster_means[:, k] .+= X_aug[:, j]
                    cluster_counts[k]   += 1
                    seen[orig_idx[j]]    = true
                end
            end
            if cluster_counts[k] > 0
                cluster_means[:, k] ./= cluster_counts[k]
            end
        end
        return cluster_means, cluster_counts, codes_aug
    end
end

# ╔═╡ 71d60ab8-3aef-11f1-87c0-11f707fe31c8
real_result = @use_memo([reload_real, vqvae_params_real, training_para_real]) do
    ...
    # Post-training: code assignment for original + cluster averages for augmented
    codes_raw = get_codes(model, xpu(X_real_raw))
    cluster_means, cluster_counts, codes_aug =
        get_cluster_averages(model, X_aug_norm, vqvae_params_real.K, orig_idx_aug)
    @info "Cluster counts" cluster_counts

    (; model, lhist, X_aug_norm, codes_raw, cluster_means, cluster_counts, codes_aug)
end

# ╔═╡ 71d60e32-3aef-11f1-994b-e9b404871738
let
    r      = real_result
...
# ╔═╡ 466e1012-3afb-11f1-8337-d74653b25f4e
md"### Real Data: Cluster Averages (Augmented Data)"

# ╔═╡ 466e1062-3afb-11f1-a6b0-8d3b9526edb4
let
    r  = real_result
    K  = vqvae_params_real.K
    ts = 1:nt_real
    cluster_means, cluster_counts = r.cluster_means, r.cluster_counts
    colors = ["#1f77b4","#ff7f0e","#2ca02c","#d62728","#9467bd","#8c564b","#e377c2","#7f7f7f"]
    traces = [PlutoPlotly.scatter(x=ts, y=cluster_means[:, k], mode="lines",
        name="Code $k  (N=$(cluster_counts[k]))",
        line=attr(color=colors[mod1(k, length(colors))], width=2))
        for k in 1:K]
	push!(traces, PlutoPlotly.scatter(x=ts, y=vec(mean(r.X_aug_norm, dims=2)), mode="lines",
        name="Full average",
        line=attr(color="black", width=2)))
    layout = Layout(
        title=attr(text="Cluster mean waveforms (augmented data) — real data"),
        xaxis=attr(title="Sample"), yaxis=attr(title="Amplitude"),
        height=360, width=900, plot_bgcolor="white", paper_bgcolor="white",
    )
    WideCell(PlutoPlotly.plot(traces, layout))
end

# ╔═╡ 8f675a8e-ccc3-4f23-9e5e-66f6e31883f6
mean(real_result.X_aug_norm, dims=2)

# ╔═╡ 65fbe8e0-3afc-11f1-9850-df7e26bcf223
let
    snr0   = snr_levels[1]         # use first SNR for preview
    X_raw_prev, _ = make_ricker_syn(; snr=snr0)
    grid_prev = make_shift_grid(nt_syn)
...
# ╔═╡ 65fbf22c-3afc-11f1-ac77-3746fd69849b
reload_syn_button = @bind reload_syn CounterButton("Reload Synthetic Models")

# ╔═╡ 71d5f534-3aef-11f1-8688-671fdf883f79
syn_results = @use_memo([reload_syn, vqvae_params_syn, training_para_syn]) do
    ...
    for snr in snr_levels
        ...
        # Post-training: code assignment for original + cluster averages for augmented
        codes_raw = get_codes(model, xpu(X_raw))
        cluster_means, cluster_counts, codes_aug =
            get_cluster_averages(model, X_aug_norm, vqvae_params_syn.K, orig_idx_aug)
        @info "SNR=$snr" cluster_counts

        results[snr] = (; model, lhist, tau_true, X_raw, X_aug_norm,
                          codes_raw, cluster_means, cluster_counts, codes_aug)
    end
    results
end

# ╔═╡ a5928ffd-4e4b-4023-9462-ba51175a2246
let
    ...
    traces = [PlutoPlotly.scatter(x=ts, y=cluster_means[:, k], mode="lines",
        ...
        for k in 1:K]
    layout = Layout(
        title=attr(text="Cluster mean waveforms (augmented data) — SNR=$snr"),
        xaxis=attr(title="Sample"), yaxis=attr(title="Amplitude"),
        height=360, width=900, plot_bgcolor="white", paper_bgcolor="white",
    )
    WideCell(PlutoPlotly.plot(traces, layout))
...
# ╔═╡ 71d5f992-3aef-11f1-8436-d9148ea1af54
let
    snr = parse(Float64, snr_sel)
    r   = syn_results[snr]
    K   = vqvae_params_syn.K
    counts_raw = [count(==(k), r.codes_raw) for k in 1:K]
    counts_aug = [count(==(k), r.codes_aug) for k in 1:K]
    bar_raw = PlutoPlotly.bar(x=1:K, y=counts_raw,
        name="Original ($N_syn waveforms)",
        marker=attr(color="#1f77b4"))
    bar_aug = PlutoPlotly.bar(x=1:K, y=counts_aug,
        name="Augmented ($(N_syn*M_aug_syn) waveforms)",
        marker=attr(color="#d62728", opacity=0.7))
    layout = Layout(
        title=attr(text="Waveforms per cluster — SNR=$snr"),
        xaxis=attr(title="Code k"), yaxis=attr(title="Count"),
        barmode="group",
        height=300, width=700, plot_bgcolor="white", paper_bgcolor="white",
    )
    PlutoPlotly.plot([bar_raw, bar_aug], layout)
end

# ╔═╡ 71d5fc9c-3aef-11f1-b771-092bc05a4c16
let
    ...
    for (i, snr) in enumerate(snr_levels)
        ...
        push!(traces, PlutoPlotly.scatter(x=ts, y=stack_raw, mode="lines",
            name="Raw mean SNR=$snr", line=attr(color=colors[i], width=1.5)))
    end
    layout = Layout(
        title=attr(text="Raw mean stacks (multiple SNRs)", font=merge(font, attr(size=16))),
...
# ╔═╡ 71d60018-3aef-11f1-aabc-657c3cbbdc08
let
    snr    = parse(Float64, snr_sel)
    r      = syn_results[snr]
    epochs = eachindex(r.lhist.train_recon)
...
# ╔═╡ 65fc07f8-3afc-11f1-a8aa-6f99812de252
md"### Data Preview: Raw vs Augmented (Real Data)"

# ╔═╡ 65fc0834-3afc-11f1-af2f-b16ec3a11fed
let
    grid_prev = make_shift_grid(nt_real)
    X_aug_prev, tau_aug_prev, orig_idx_prev, _ =
        augment_with_shifts(X_real_raw, M_aug_real, max_shift_real, grid_prev)
    X_aug_prev = normalise_traces(X_aug_prev)
    ts     = 1:nt_real
    ...
    # --- left: raw traces ---
    idxs_raw = round.(Int, range(1, N_real; length=n_show))
    traces_raw = [PlutoPlotly.scatter(
        x=ts, y=X_real_raw[:, i] .+ (j-1)*scale,
        mode="lines", showlegend=false,
        line=attr(color="#1f77b4", width=0.8), opacity=0.7)
        for (j, i) in enumerate(idxs_raw)]

    # --- right: all M shifts of waveform 1 ---
    idxs_aug = findall(==(1), orig_idx_prev)
    traces_aug = [PlutoPlotly.scatter(
        x=ts, y=X_aug_prev[:, idxs_aug[j]] .+ (j-1)*scale,
        mode="lines", showlegend=false,
        name="τ=$(round(tau_aug_prev[idxs_aug[j]]; digits=1))",
    ...
    p_raw = PlutoPlotly.plot(traces_raw, Layout(
        title=attr(text="Raw ($N_real traces)"),
        xaxis=attr(title="Sample"), yaxis=attr(showticklabels=false),
        height=450, width=440, plot_bgcolor="white", paper_bgcolor="white"))
    p_aug = PlutoPlotly.plot(traces_aug, Layout(
        title=attr(text="All M=$M_aug_real shifts of 1 waveform (augmented)"),
        xaxis=attr(title="Sample"), yaxis=attr(showticklabels=false),
        height=450, width=440, plot_bgcolor="white", paper_bgcolor="white"))
    [p_raw p_aug]
end

# ╔═╡ 9fc78fb2-3b05-11f1-913b-53026e35e34f
md"### Real Data: Waveforms per Cluster"

# ╔═╡ 9fc79002-3b05-11f1-86a3-4b8e469a9f47
let
    r  = real_result
    K  = vqvae_params_real.K
    counts_raw = [count(==(k), r.codes_raw) for k in 1:K]
    counts_aug = [count(==(k), r.codes_aug) for k in 1:K]
    bar_raw = PlutoPlotly.bar(x=1:K, y=counts_raw,
        name="Original ($N_real waveforms)",
        marker=attr(color="#1f77b4"))
    bar_aug = PlutoPlotly.bar(x=1:K, y=counts_aug,
        name="Augmented ($(N_real*M_aug_real) waveforms)",
        marker=attr(color="#d62728", opacity=0.7))
    layout = Layout(
        title=attr(text="Waveforms per cluster — real data"),
        xaxis=attr(title="Code k"), yaxis=attr(title="Count"),
        barmode="group",
        height=300, width=700, plot_bgcolor="white", paper_bgcolor="white",
    )
