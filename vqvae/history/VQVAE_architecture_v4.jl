...
Base.@kwdef struct VQVAE_Para
    ...
    # v4 multiscale quantization: coarse smooth codebook + residual detail codebooks
    use_multiscale_rvq::Bool = true
    Ksmall::Int = 8                         # stage-1 coarse codebook size
    Klarge::Int = 16                        # stage-2+ detail codebook size
    detail_stages::Int = 1                  # number of residual detail stages (>=1)
    ema_decay_small::Float32 = 0.999f0      # slower EMA for coarse anchors
    ema_decay_large::Float32 = 0.99f0       # faster EMA for detail residuals

    # Cosine threshold + max-neighbor cap smoothing (hybrid)
    smooth_cos_threshold::Float32 = 0.8f0
    smooth_max_neighbors::Int = 16
    smooth_min_neighbors::Int = 4
    smooth_temperature::Float32 = 0.1f0
    smooth_blend::Float32 = 1.0f0           # 1.0 => fully smoothed coarse input, 0.0 => raw latent
end
...
begin
    ...
    function MultiscaleRVQQuantizer(Ksmall::Int, Klarge::Int, d::Int;
        detail_stages::Int=1,
        ...
        smooth_min_neighbors::Int=4,
        smooth_temperature::Float32=0.1f0,
        smooth_blend::Float32=1.0f0)
        detail_stages >= 1 || error("detail_stages must be >= 1, got $detail_stages")
        smooth_max_neighbors >= 1 || error("smooth_max_neighbors must be >= 1")
        smooth_min_neighbors >= 1 || error("smooth_min_neighbors must be >= 1")
        smooth_blend = clamp(smooth_blend, 0f0, 1f0)
        coarse = VectorQuantizerEMA(Ksmall, d; decay=decay_small, epsilon=epsilon, dead_threshold=dead_threshold)
        detail = [VectorQuantizerEMA(Klarge, d; decay=decay_large, epsilon=epsilon, dead_threshold=dead_threshold)
                  for _ in 1:detail_stages]
        return MultiscaleRVQQuantizer(coarse, detail, Ksmall, d,
            smooth_cos_threshold, smooth_max_neighbors, smooth_min_neighbors,
            smooth_temperature, smooth_blend)
    end
    ...
    function (vq::MultiscaleRVQQuantizer)(z_e::AbstractMatrix{Float32}; beta_commit::Float32=0.25f0, training::Bool=true)
        ...
        W = Zygote.@ignore cosine_neighbor_weights(z_e;
            threshold=vq.smooth_cos_threshold,
            max_neighbors=vq.smooth_max_neighbors,
            min_neighbors=vq.smooth_min_neighbors,
            temperature=vq.smooth_temperature)

        z_smooth = z_e * W
        z_coarse_in = vq.smooth_blend .* z_smooth .+ (1f0 - vq.smooth_blend) .* z_e
...
begin
    ...
    function get_multiscale_codebooks(m)
        if !(m.quantizer isa MultiscaleRVQQuantizer)
...
- `training`: whether to update EMA codebook
"""
function vqvae_forward(model, x, beta_commit::Float32, training::Bool)
    return model(x; beta_commit=beta_commit, training=training)
end

# ╔═╡ 8646155e-376f-44e6-89df-c3092d350046
function vqvae_total_loss(result, x, entropy_weight::Float32)
...
function loss_vqvae(model, x, para; training::Bool=true)
    result = vqvae_forward(model, x, para.beta_commit, training)
    total, recon_loss, commit_loss, entropy_loss = vqvae_total_loss(result, x, para.entropy_weight)
    return (; total, recon_loss, commit_loss, entropy_loss,
...
function vq_eval_metrics(model, x_tr, x_te, para)
    train_m = loss_vqvae(model, x_tr, para; training=false)
    test_m  = loss_vqvae(model, x_te, para; training=false)
...
function vq_train_epoch!(model, opt_state, train_loader, para)
    for x in train_loader
        g = Flux.gradient(model, x) do m, xb
            loss_vqvae(m, xb, para; training=true).total
        end
...
function train_coarse_to_detail_classifier(coarse_codes::Vector, detail_codes::Matrix; 
    ...
    Ksmall = maximum(coarse_codes)        # Coarse codebook size
    T_detail = size(detail_codes, 1)      # Detail trajectory length
    Klarge = maximum(detail_codes)        # Detail codebook size
    ...
    if T_detail == 1
        ...
        combo_mult = cumprod(vcat(1, fill(Klarge, T_detail-1)))
        for j in 1:n_samples
            target[j] = sum((detail_codes[:, j] .- 1) .* combo_mult) + 1  # 1-indexed
        end
        n_classes = Klarge^T_detail
    end
    ...
    X = Flux.onehotbatch(coarse_codes, 1:Ksmall)  # (Ksmall × n_samples)
    ...
    model = Chain(
        Dense(Ksmall, n_classes, identity)
    ) |> gpu
...
"""
function per_position_prototype_snr(model, x, para)
    proto = per_position_cluster_averages(model, x)
...
"""
function per_latent_slot_entropy(model, x)
    res = encode(model, x)
...
function get_vqvae(para)
    ...
    quantizer = if para.use_multiscale_rvq
        ...
        MultiscaleRVQQuantizer(para.Ksmall, para.Klarge, para.d;
            ...
            smooth_cos_threshold=para.smooth_cos_threshold,
            smooth_max_neighbors=para.smooth_max_neighbors,
            smooth_min_neighbors=para.smooth_min_neighbors,
            smooth_temperature=para.smooth_temperature,
            smooth_blend=para.smooth_blend)
    elseif para.per_slot_codebook
        ...
        PerSlotVectorQuantizer(actual_T, para.K, para.d;
            decay=para.ema_decay, epsilon=para.epsilon,
            dead_threshold=para.dead_threshold)
    else
        VectorQuantizerEMA(para.K, para.d;
            decay=para.ema_decay, epsilon=para.epsilon,
            dead_threshold=para.dead_threshold)
    end
