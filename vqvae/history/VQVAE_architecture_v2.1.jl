...
- **Slot attention pooling** gives each code a learned temporal focus
- **EMA codebook updates** (no gradient through codebook — more stable)
- **Configurable T** (number of slots / quantized vectors per waveform) controls how many arrivals can be represented
...
Base.@kwdef struct VQVAE_Para
    ...
    T::Int = 2                             # slot count / quantized vectors per waveform
    ...
    attention_entropy_weight::Float32 = 0.0f0   # negative entropy on slot attention to sharpen peaks
    attention_diversity_weight::Float32 = 0.01f0  # penalize overlapping slot attention maps
    seed = nothing
end
...
begin
    struct VQVAE{E,P,A,VQ,D}
        encoder::E         # Conv1DChain: waveform → feature map (L, C, B)
        pre_vq::P          # 1x1 Conv: feature map → latent map (L, d, B)
        attention_head::A  # 1x1 Conv: latent map → slot logits (L, T, B)
        quantizer::VQ      # VectorQuantizerEMA
        decoder::D         # Conv1DChain: (d * T) → waveform
        T::Int             # number of quantized slot vectors per waveform
        d::Int             # codebook embedding dimension
    end
    Flux.@layer VQVAE trainable = (encoder, pre_vq, attention_head, decoder)

    function pool_slots(m::VQVAE, z_map::AbstractArray{Float32,3})
        ...
        # Avoid in-place GPU writes in AD path: batched (d,L,B) * (L,T,B) -> (d,T,B).
        ...
        # Per-batch slot-overlap matrices: (T,L,B) * (L,T,B) -> (T,T,B), then batch-average.
        overlap_batch = NNlib.batched_mul(attn_bt, attn_weights)
    ...
    function (m::VQVAE)(x; beta_commit::Float32=0.25f0, training::Bool=true)
        ...
        z_q_for_dec = reshape(vq_result.z_q, m.d * m.T, N_total)
        xhat = m.decoder(z_q_for_dec)

        codebook_indices = reshape(cpu(vq_result.indices), m.T, N_total)
    ...
    function encode(m, x)
        ...
        codebook_indices = reshape(cpu(vq_result.indices), m.T, N_total)
        z_q = reshape(vq_result.z_q, m.d, m.T, N_total)
        return (; z_map, z_e=reshape(z_e, m.d, m.T, N_total), z_q, codebook_indices,
    ...
    """
    get_codebook(m) = cpu(m.quantizer.embedding)
...
function get_vqvae(para::VQVAE_Para)
    ...
    # Encoder preserves the latent time axis for slot attention pooling.
    ...
    model = VQVAE(xpu(encoder), xpu(pre_vq), xpu(attention_head), quantizer, xpu(decoder), para.T, para.d)
