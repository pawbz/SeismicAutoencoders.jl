...
begin
    ...
    condition_dim_syn = 1  # normalized scalar condition from EventLoc[1]
    shift_input_mode_syn = :condition  # choose from :waveform, :condition, :both
    smoothness_regime_syn = :shift_plus_condition  # :none | :shift_only | :shift_plus_condition
    shift_regularization_gamma_syn = 0.000f0
    condition_smoothness_gamma_syn = 0.1f0
    if smoothness_regime_syn == :none
        shift_regularization_gamma_syn = 0f0
        condition_smoothness_gamma_syn = 0f0
    elseif smoothness_regime_syn == :shift_only
        condition_smoothness_gamma_syn = 0f0
    elseif smoothness_regime_syn == :shift_plus_condition
        # keep both terms on
    else
        error("Unknown smoothness_regime_syn: $smoothness_regime_syn")
    end
    condition_smoothness_k_syn = 8
...
# ╔═╡ fa0dae2d-9d67-4caa-855e-bfd474a13608
...
fldir="/mnt/NAS/EQData/RFData" # mounted NAS1 on Chuong system 
...
cond_syn_raw = reshape(Float32.(getindex.(EvtLoc[1], 1)), 1, :)
...
vqvae_syn_parameters = (;
    nt=nt_syn,
    d=5,
    K=3,             # small codebook: only ~2-3 "shapes" in Ricker data (same polarity etc.)
    T=1,
    beta_commit=0.25f0,
    ema_decay=0.99f0,
    ...
    condition_smoothness_gamma=condition_smoothness_gamma_syn,
    condition_smoothness_k=condition_smoothness_k_syn,
    # ── Spatial transformer ──────────────────────────────────────────────
    gamma=shift_regularization_gamma_syn,
    max_shift_samples=round(Int, max_shift_syn / dt_syn) + 25,  # a bit wider than true range
    shift_penalty_type=:l2,
    cauchy_sigma=Float32(max_shift_syn / dt_syn) / 2f0,
    seed=42,
)
...
@bind iatom Slider(1:vqvae_syn_parameters.K, show_value=true)
...
X_syn = normalise(onestation_data, dims=1)
...
begin
    ...
    WideCell(
        ...
        title="Synthetic Ricker Test ($(shift_input_mode_syn), $(smoothness_regime_syn), heavy noise) — VQ-VAE ST Training")
	)
end
...
let
    trained_syn
    xhat3 = cpu(model_syn(X_syn_train; condition=cond_syn_train).xhat[:, 1:3])
...
let
    ...
    layout = Layout(
        ...
        xaxis=attr(title="Condition (raw EventLoc[1])"),
