...
vqvae = @ingredients("/mnt/NAS/EQData/SeismicAutoencoders/VQVAE_architecture_v2.jl")
...
vqvae_parameters = (;
    ...
    T = 2,             # quantized vectors per waveform
    ...
    dead_threshold = 10,     # ↓ from 2: reset dead entries after 1 batch unused
...
model, loss_history = @use_memo([reload_network]) do
    reload_network
...
trained = @use_memo([]) do
    training_para = vqvae.VQVAE_Training_Para(
        batchsize = 96,
        nepoch = 25,
        initial_learning_rate = 0.01,  # ↑ from 0.001,
...
let
    ...
    x_ac_comps = cpu(r_ac.xhat)  # (nt, nac, 1, T)
    x_c_comps  = cpu(r_c.xhat)  # (nt, nc, 1, T)

	
    r_ac=normalise(cpu(vec(mean(r_ac.xhat, dims=2))))
    r_c=normalise(cpu(vec(mean(r_c.xhat, dims=2))))
    ac=normalise(cpu(vec(mean(x_ac_sel; dims=2))))
    c=normalise(cpu(vec(mean(x_c_sel; dims=2))))
    (; raw=[reverse(ac); c], recon = [reverse(r_ac); r_c])
    ...
    colors = [Colors.hex(get(cs, (i - 1) / max(1, nc - 1))) for i in 1:nc]
        ...
        offset = (i - 1) * 3.0
            c = colors[i]
            push!(traces, PlutoPlotly.scatter(x=t, y=averaged_combinations[i].raw .* 0.5 .+ offset,         mode="lines", line=attr(color="black", width=2), opacity=0.5,
                showlegend=(i==1), name="Raw $(combo_labels[i])"))
      
        push!(traces, PlutoPlotly.scatter(x=t, y=averaged_combinations[i].recon .* 0.5 .+offset,
