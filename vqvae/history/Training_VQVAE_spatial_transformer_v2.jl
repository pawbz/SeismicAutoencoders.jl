...
md"""# VQ-VAE + Discrete Shift Codebook — Training Notebook

Train a **VQ-VAE with fixed discrete time-shift alignment** on either
...
vqvae = @ingredients("/mnt/NAS/EQData/SeismicAutoencoders/VQVAE_spatial_transformer_v2.jl")
...
vqvae_syn_parameters = (;
    nt=nt_syn,
    d=5,
    K=1,             # small codebook: only ~2-3 "shapes" in Ricker data (same polarity etc.)
    T=1,
    beta_commit=0.25f0,
    ema_decay=0.99f0,
    ...
    # ── Spatial transformer ──────────────────────────────────────────────
    max_shift_samples=round(Int, max_shift_syn / dt_syn) + 25,  # fixed physical search range
    seed=42,
)
...
X_syn = normalise(taper(onestation_data), dims=1)
