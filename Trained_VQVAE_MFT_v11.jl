...
# ╔═╡ 5fcbcdde-f640-4b84-9ef8-bf0f6bd781ef
using PlutoPlotly
...
begin
    @bind _mft_plot_controls PlutoUI.combine() do Child
        ...
        | Phase velocity min (km/s) | $(Child("phase_vmin", NumberField(1.0:0.1:8.0; default=1.0))) |
        | Phase velocity max (km/s) | $(Child("phase_vmax", NumberField(1.0:0.1:8.0; default=6.0))) |
    ...
    for pair_dir in sort(filter(isdir, readdir(saved_root, join=true)))
        ...
        for run_dir in sort(filter(isdir, readdir(pair_dir, join=true)))
            ...
            push!(runs, (; pair=(String(parts[1]), String(parts[2])),
                ...
                run_dir, artifact_path=joinpath(run_dir, "source_state_averages.jld2"),
...
function _source_state_artifact_path(run)
    hasproperty(run, :artifact_path) ? String(run.artifact_path) :
...
begin
    ...
    @bind _pair_mode_controls confirm(PlutoUI.combine() do Child
        md"""
        | Computation selection | Value |
        |:---|:---|
        | Selected pairs | $(Child("pairs", MultiCheckBox(_pair_options_ui; default=rand(_selected_pairs_default_ui, 5), select_all=true))) |
        """
    end)
end
...
selected_runs = begin
    ...
    keep_only_latest_rerun_per_seed = false
...
# ╔═╡ 51cde60c-7da1-46c9-a20e-d93af9741427
...
selected_source_state_cache = let
    reload_saved_artifacts_button
    runs_to_load = sort(unique(vcat(selected_runs, selected_globalavg_runs)); by=run -> String(run.run_dir))
    cache = Dict{String,Any}()
    @progress name="Loading selected source-state artifacts once" for run in runs_to_load
        cache[String(run.run_dir)] = _load_saved_source_state_averages(run)
    end
    cache
end;
...
begin
    ...
    analysis_settings = isempty(selected_source_state_cache) ?
        nothing : first(values(selected_source_state_cache)).analysis_settings
...
# ╔═╡ 3f3b4fc7-fb3b-40f2-96d1-14d8c293765a
run_source_state_averages = [selected_source_state_cache[String(run.run_dir)] for run in selected_runs];

# ╔═╡ 1f2f5740-5bd0-11f1-0028-4b3f11e7a001
globalavg_source_state_averages = [selected_source_state_cache[String(run.run_dir)] for run in selected_globalavg_runs];
...
# ╔═╡ de03ff6d-3844-4259-bbbc-1bbfd484ea0f
...
# ╔═╡ 8130ba45-4400-46a1-977f-1a6f350b67da
...
# ╔═╡ 01575adb-e1c4-44f7-a79f-340b2320af33
...
# ╔═╡ cd2e0828-8195-4b33-b24e-bcacd3dd1b1f
...
# ╔═╡ 76acb748-1c40-4dbb-b275-8b57c0920852
...
# ╔═╡ df62394b-4252-4f3e-a7fe-80a1c4f180d2
function _tex_escape_text(s)
    replace(String(s),
        "\\" => "\\textbackslash{}",
        "{" => "\\{",
        "}" => "\\}",
...
# ╔═╡ aff2041b-a329-421f-a8ec-b35db0387fce
function _make_empty_uc_pgfplot(pair_label::String, message::String)
    title_text = "\\shortstack{\\textbf{$(_tex_escape_text(pair_label))}\\\\{\\footnotesize $(_tex_escape_text(message))}}"
    axis = PGFPlotsX.Axis(
        PGFPlotsX.Options(
            "width" => "16cm",
            "height" => "5cm",
            ...
            "grid" => "major"),
        PGFPlotsX.Plot(PGFPlotsX.Options("draw" => "none"),
            PGFPlotsX.Table([:x => [1.0, 2.0], :y => [1.0, 1.0]])))
    PGFPlotsX.TikzPicture(axis)
end

# ╔═╡ 6080d51d-fce9-45e2-9d53-ed00ce9ad36f
function _pgf_plot_options(; color::AbstractString, marker::AbstractString,
        line_style::AbstractString="solid", line_width::AbstractString="1.2pt",
        marker_options::AbstractString="{solid,fill=white,draw=black,line width=0.25pt}",
        forget::Bool=false)
    opts = PGFPlotsX.Options(
        "color" => String(color),
        "mark" => String(marker),
        "line width" => String(line_width),
        "mark options" => String(marker_options))
    if line_style == "dashed"
        opts["dashed"] = nothing
    elseif line_style == "dotted"
        opts["dotted"] = nothing
    end
    forget && (opts["forget plot"] = nothing)
    opts
end

# ╔═╡ 56330d1c-1706-4603-8791-e5ad695c5df4
function _add_pgf_series!(items::Vector{Any}, rows, field::Symbol;
        label::String, color::AbstractString, marker::AbstractString,
        line_style::AbstractString="solid", line_width::AbstractString="1.2pt",
        marker_options::AbstractString="{solid,fill=white,draw=black,line width=0.25pt}",
        legend::Bool=true)
    xs, ys = _series_xy(rows, field)
    isempty(xs) && return nothing
    push!(items, PGFPlotsX.Plot(
        _pgf_plot_options(; color, marker, line_style, line_width, marker_options),
        PGFPlotsX.Table([:x => xs, :y => ys])))
    legend && push!(items, PGFPlotsX.LegendEntry(label))
    nothing
end

# ╔═╡ 582c0e7d-8212-4435-83dc-68e3bff42517
function _add_pgf_error_series!(items::Vector{Any}, rows;
        color::AbstractString, marker::AbstractString,
        line_style::AbstractString="solid", line_width::AbstractString="1.0pt",
        marker_options::AbstractString="{solid,fill=white,draw=black,line width=0.25pt}",
        error_cap::Real=0.5)
    xs, ys = _series_err_xy(rows; error_cap)
    isempty(xs) && return nothing
    push!(items, PGFPlotsX.Plot(
        _pgf_plot_options(; color, marker, line_style, line_width,
            marker_options, forget=true),
        PGFPlotsX.Table([:x => xs, :y => ys])))
    nothing
end

# ╔═╡ 0115e073-87f3-4578-90a4-82315f94ac01
function _pgf_axis_period_limits(rows)
    periods = Float64[
        Float64(r.period) for r in rows
        if hasproperty(r, :period) && isfinite(Float64(r.period)) && Float64(r.period) > 0.0]
    isempty(periods) && return (1.0, 10.0)
    lo, hi = extrema(periods)
    (lo * 0.98, hi * 1.02)
end

# ╔═╡ a4afc10d-ce8f-457a-908f-7e77e0babee8
function _pgf_velocity_limits(rows, velocity_range)
    if !isnothing(velocity_range)
        return (Float64(velocity_range[1]), Float64(velocity_range[2]))
    end
    vals = Float64[]
    for row in rows
        for field in (:group_velocity, :predicted_group_velocity, :phase_velocity)
            hasproperty(row, field) || continue
            v = Float64(getproperty(row, field))
            isfinite(v) && v > 0.0 && push!(vals, v)
        end
    end
    isempty(vals) && return (0.5, 8.0)
    lo, hi = extrema(vals)
    (max(0.0, lo * 0.92), hi * 1.08)
end

# ╔═╡ 8110dae3-9c0e-4c6a-8cf5-6e563bb5c53f
function _write_uc_consistency_pdf(path::AbstractString, fig::PGFPlotsX.TikzPicture)
    isempty(strip(path)) && error("Set a PDF output path before writing.")
    mkpath(dirname(path))
    PGFPlotsX.pgfsave(path, fig)
    path
end

# ╔═╡ 20b37ee0-b523-4568-a753-c290a16e84c3
function _write_uc_consistency_png(path::AbstractString, fig::PGFPlotsX.TikzPicture; dpi::Integer=180)
    isempty(strip(path)) && error("Set a PNG output path before writing.")
    mkpath(dirname(path))
    PGFPlotsX.pgfsave(path, fig; dpi)
    path
end
...
# ╔═╡ 670285fd-9c14-4357-84f2-251dd8ffc265
...
# ╔═╡ 48fbad3f-a4e7-4d24-904c-c530c86a9bdc
...
# ╔═╡ dea84559-a3e8-4000-a78a-12ef5a875b48
...
# ╔═╡ fc2cd9e2-bc85-4146-863b-c990c836b361
function _make_uc_consistency_pgfplot(global_rows, overlay_rows, pair_label::String;
    ...
    isempty(pair_global_rows) && return _make_empty_uc_pgfplot(pair_label,
    ...
    title_text = "\\shortstack{\\textbf{$(_tex_escape_text(pair_label)) | $(_tex_escape_text(distance_label))}\\\\{\\footnotesize $(_tex_escape_text(selected_label))}\\\\{\\footnotesize $(_tex_escape_text("U-c $(score_method): global $(_format_branch_scores(global_scores)); selected $(_format_branch_scores(selected_scores))"))}}"

    all_pair_rows = vcat(pair_global_rows, pair_overlay_rows)
    xmin, xmax = _pgf_axis_period_limits(all_pair_rows)
    ymin, ymax = _pgf_velocity_limits(all_pair_rows, velocity_range)
    gp = PGFPlotsX.GroupPlot(PGFPlotsX.Options(
        "group style" => "{group size=1 by 2, vertical sep=2.0cm, xlabels at=edge bottom, xticklabels at=edge bottom}",
        "width" => "16cm",
        "height" => "6.6cm",
        "xmin" => xmin,
        "xmax" => xmax,
        "grid" => "both",
        "major grid style" => "{line width=.25pt,draw=gray!35}",
        "minor grid style" => "{line width=.15pt,draw=gray!18}",
        "tick align" => "outside",
        "tick style" => "{black,line width=.35pt}"))

    vel_items = Any[]
    err_items = Any[]
    global_style = Dict(
        "causal" => (; color="blue!70!black", marker="*", open_marker="o", phase_marker="square"),
        "acausal" => (; color="red!75!black", marker="diamond*", open_marker="diamond", phase_marker="square"))
    selected_style = Dict(
        "causal" => (; color="green!45!black", marker="star", open_marker="triangle", phase_marker="square"),
        "acausal" => (; color="violet!80!black", marker="star", open_marker="triangle", phase_marker="square"))

    for branch in ("causal", "acausal")
        rows_b = _rows_for_pair_and_branch(global_rows, pair_label, branch)
        style = global_style[branch]
        _add_pgf_series!(vel_items, rows_b, :group_velocity;
            label="global $(branch) U", color=style.color, marker=style.marker,
            line_width="1.15pt",
            marker_options="{solid,fill=$(style.color),draw=black,line width=0.2pt}")
        _add_pgf_series!(vel_items, rows_b, :predicted_group_velocity;
            label="global $(branch) U(c)", color=style.color, marker=style.open_marker,
            line_style="dashed", line_width="0.95pt",
            marker_options="{solid,fill=white,draw=$(style.color),line width=0.35pt}")
        _add_pgf_series!(vel_items, rows_b, :phase_velocity;
            label="global $(branch) c", color=style.color, marker=style.phase_marker,
            line_style="dotted", line_width="0.85pt",
            marker_options="{solid,fill=white,draw=$(style.color),line width=0.35pt}")
        _add_pgf_error_series!(err_items, rows_b;
            color=style.color, marker=style.marker, line_width="0.8pt",
            marker_options="{solid,fill=$(style.color),draw=black,line width=0.2pt}",
            error_cap)
    end
    ...
    for branch in ("causal", "acausal")
        ...
        _add_pgf_series!(vel_items, rows_b, :group_velocity;
            label="selected $(branch) U", color=style.color, marker=style.marker,
            line_width="1.35pt",
            marker_options="{solid,fill=$(style.color),draw=black,line width=0.2pt}")
        _add_pgf_series!(vel_items, rows_b, :predicted_group_velocity;
            label="selected $(branch) U(c)", color=style.color, marker=style.open_marker,
            line_style="dashed", line_width="1.05pt",
            marker_options="{solid,fill=white,draw=$(style.color),line width=0.35pt}")
        _add_pgf_series!(vel_items, rows_b, :phase_velocity;
            label="selected $(branch) c", color=style.color, marker=style.phase_marker,
            line_style="dotted", line_width="0.9pt",
            marker_options="{solid,fill=white,draw=$(style.color),line width=0.35pt}")
        _add_pgf_error_series!(err_items, rows_b;
            color=style.color, marker=style.marker, line_style="dashed",
            line_width="0.85pt",
            marker_options="{solid,fill=$(style.color),draw=black,line width=0.2pt}",
            error_cap)
    end

    push!(gp, PGFPlotsX.Options(
        "title" => title_text,
        "title style" => "{align=center,font=\\small}",
        "ylabel" => "Velocity (km/s)",
        "ymin" => ymin,
        "ymax" => ymax,
        "legend style" => "{at={(0.5,-0.17)},anchor=north,legend columns=3,font=\\scriptsize,draw=gray!30,fill=white,/tikz/every even column/.append style={column sep=0.25cm}}",
    ...
    push!(gp, PGFPlotsX.Options(
        ...
        "ymin" => 0.0,
        "ymax" => Float64(error_cap)),
        err_items...)
    PGFPlotsX.TikzPicture(gp)
end
...
# ╔═╡ eb328033-d92e-4899-82d3-24a950175d25
begin
    _plot_pair_values = sort(String.(collect(_pair_mode_controls.pairs)))
    filter!(!=("None"), _plot_pair_values)
    _plot_pair_best_score(pl) = begin
        specs = get(_all_scored_specs_by_pair, String(pl), NamedTuple[])
        scores = Float64[Float64(s.uc_score) for s in specs
            if hasproperty(s, :uc_score) && isfinite(Float64(s.uc_score))]
        isempty(scores) ? Inf : minimum(scores)
    end
    _plot_pair_distance_label(pl) = begin
        d = _pair_distance_for_title(String(pl))
        isfinite(d) ? "$(round(d; digits=1)) km" : "NA"
    end
    _plot_pair_score_label(pl) = begin
        score = _plot_pair_best_score(pl)
        isfinite(score) ? string(round(score; digits=3)) : "NA"
    end
    sort!(_plot_pair_values; by=pl -> (_plot_pair_best_score(pl), String(pl)))
    _plot_pair_options_ui = isempty(_plot_pair_values) ?
        ["None" => "None"] :
        [pl => "$(pl) | distance=$(_plot_pair_distance_label(pl)) | best score=$(_plot_pair_score_label(pl))"
         for pl in _plot_pair_values]
    _plot_pair_default = isempty(_plot_pair_values) ? "None" : first(_plot_pair_values)
    @bind _pair_mode_controls_plot PlutoUI.combine() do Child
        md"""
        | Computation selection | Value |
        |:---|:---|
        | Plot pair | $(Child("plot_pair", Select(_plot_pair_options_ui; default=_plot_pair_default))) |
        """
    end
end
...
if @isdefined(selected_plot_pair)
    _uc_pgf_fig = _make_uc_consistency_pgfplot(
    ...
    _uc_png_path = joinpath(tempdir(),
        "v11_uc_consistency_$(_safe_filename(selected_plot_pair))_$(_safe_filename(selected_marginal_combo_overlay)).png")
    try
        _write_uc_consistency_png(_uc_png_path, _uc_pgf_fig; dpi=180)
        WideCell(PlutoUI.LocalResource(_uc_png_path))
    catch err
        md"PGFPlotsX preview failed: `$(sprint(showerror, err))`"
    end
else
    md""
end
...
let
    if !(@isdefined(_uc_pdf_controls)) || Int(_uc_pdf_controls.write_pdf) == 0
        md"Press **Write PDF** to export the current PGFPlotsX U-c consistency figure."
    else
        fig = _make_uc_consistency_pgfplot(
            global_average_uc_rows,
            selected_mft_waveform_combined_overlay_rows,
            selected_plot_pair;
            selected_spec=selected_mft_overlay_spec,
...
manifest_format = "2.0"
project_hash = "b80bee0450a542a7130b1203fd510b2a69c1d67d"
