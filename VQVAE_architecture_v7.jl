...
function get_vqvae(para::VQVAE_Para; rng=Random.default_rng(), device=identity)
    ...
    pre_vq = Dense(latent_len * enc_channels, para.d)
...
function rebuild_latent_index!(idx::LatentIndex, model, ps, st, X;
    Mnn::Int=idx.Mnn, batch_size::Int=256, latent_index_space::Symbol=:z_e,
    ...
    for start_idx in 1:batch_size:N
        ...
        enc, _ = encode(model, ps_dev, st_eval, device(X_cpu[:, cols]); training=false)
    ...
    scoring_start = time()
    embeddings_dev = device(embeddings)
    scores = Float32.(cdev(embeddings_dev' * embeddings_dev))
    ...
    @debug "Latent index rebuild timings" embedding_time_s=round(embedding_time; digits=3) scoring_time_s=round(scoring_time; digits=3) topk_time_s=round(topk_time; digits=3)
...
function eval_metrics(model, ps, st, X_cpu, weights_cpu, para::VQVAE_Para;
    ...
    x = X_cpu[:, ids]
    total, _, stats = vqvae_loss(model, ps_cpu, st_cpu, x, x, weights_cpu, para; training=false)
    ...
    for epoch in 1:training_para.nepoch
        ...
        if phase.post_epoch > 0 &&
           (phase.Mnn != last_index_Mnn || mod(phase.post_epoch - 1, training_para.index_refresh_every) == 0)
            ...
            @info "Rebuilt latent index" epoch post_warmup_epoch=phase.post_epoch Mnn=phase.Mnn aggregation=phase.aggregation latent_index_space=training_para.latent_index_space latent_index_time_s=round(latent_index_time; digits=3)
        end
