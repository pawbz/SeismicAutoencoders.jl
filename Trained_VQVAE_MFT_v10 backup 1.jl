...
begin
    ...
    _selected_marginal_combo_options = ["None"; String[
        "$(s.display) | med err=$(isfinite(s.median_relative_error) ? round(s.median_relative_error; digits=3) : "NA")"
...
function _source_state_mix_search(pair_label::String;
    ...
    for (itr, res) in enumerate(batch.state_results)
        ...
        push!(scored, (; trial_index=itr,
            ...
            stats..., active_count,
            objective=stats.max_error + 0.1 * stats.p95_error +
                0.01 * stats.median_error - 0.01 * stats.coverage_fraction,
    ...
    ranked = NamedTuple[]
    for (irank, item) in enumerate(keep)
        label = "source-state U-c mix rank $(irank)/$(length(keep)) ($(pool_mode), max=$(round(item.max_error; digits=3)), p95=$(round(item.p95_error; digits=3)), med=$(round(item.median_error; digits=3)))"
    ...
    (; rows, ranked, summary_rows, selected, basis,
        actual_trials=size(A, 2), pool_mode)
end
...
let
    ...
    if isempty(rows)
        ...
        lines = String[
            "Source-state U-c mixes for **$(selected_plot_pair)** using **$(selected_source_state_mix_payload.pool_mode)**: evaluated **$(selected_source_state_mix_payload.actual_trials)** trials.",
            "",
            "| rank | kind | valid | coverage | max err | p95 err | median err | active | top weights |",
            "|---:|:---|---:|---:|---:|---:|---:|---:|:---|",
        ]
        for row in rows
            cov = isfinite(row.coverage_fraction) ? string(round(row.coverage_fraction; digits=3)) : ""
            push!(lines, "| $(row.rank) | $(row.kind) | $(row.n_valid) | $(cov) | $(round(row.max_error; digits=3)) | $(round(row.p95_error; digits=3)) | $(round(row.median_error; digits=3)) | $(row.active_count) | $(replace(row.top_weights, "|" => "\\|")) |")
        end
        Markdown.parse(join(lines, "\n"))
    end
end
