...
begin
    _global_uc_codebook_modes = ["joint (K1×K2)", "K1+K2"]
    global_uc_codebook_overlay_specs = NamedTuple[]
    for item in global_uc_codebook_items
        for mode in _global_uc_codebook_modes
            waves, labels, family = _codebook_family_waves(item, mode)
            isempty(waves) && continue
            for wave_index in axes(waves, 2)
                wave_label = wave_index <= length(labels) ? labels[wave_index] : string(wave_index)
                display = "$(mode) | seed $(item.seed) | $(wave_label)"
                stationary_label = family == "K1+K2" ?
                    "seed$(item.seed)-$(wave_label)" :
                    "seed$(item.seed)-$(family)-$(wave_label)"
                push!(global_uc_codebook_overlay_specs, (; item, mode, family,
                    seed=Int(item.seed), wave_index=Int(wave_index),
                    wave_label=String(wave_label), stationary_label, display))
            end
        end
    end
    _global_uc_stationary_selected = [r for r in codebook_stationary_results
        if r.pair_label == selected_plot_pair]
    _global_uc_stationary_rank = Dict{String,Int}()
    if !isempty(_global_uc_stationary_selected)
        result = first(_global_uc_stationary_selected)
        ...
        for (irank, summary) in enumerate(ranked)
            _global_uc_stationary_rank[String(summary.label)] = irank
        end
    end
    sort!(global_uc_codebook_overlay_specs,
        by=s -> (get(_global_uc_stationary_rank, String(s.stationary_label), typemax(Int)),
                 s.mode, s.seed, s.wave_index, s.display))
    _global_uc_codebook_spec_options = Any[0 => "Super atom"]
    append!(_global_uc_codebook_spec_options, [
        i => global_uc_codebook_overlay_specs[i].display
        for i in eachindex(global_uc_codebook_overlay_specs)
    ])
    @bind _global_uc_overlay_controls PlutoUI.combine() do Child
        md"""
        | Global U-c plot codebook overlay | Value |
        |:---|:---|
        | Super atom mode | $(Child("super_mode", Select(["Smooth weighted U-c mix", "Band-cut U-c patches", "Bandwise U-c-guided", "Smooth low-U path"]; default="Smooth weighted U-c mix"))) |
        | Super atom family | $(Child("family", Select(_global_uc_codebook_modes; default="joint (K1×K2)"))) |
        | Bandwise U-c tolerance | $(Child("threshold", Slider(0.01:0.01:0.50; default=0.10, show_value=true))) |
        | Bandwise mix size | $(Child("mix_size", Slider(1:1:5; default=1, show_value=true))) |
        | Bandwise candidate peaks considered | $(Child("candidate_peaks", Slider(1:1:4; default=4, show_value=true))) |
        | Band-cut top mixes | $(Child("bandcut_top_mixes", Slider(1:1:20; default=5, show_value=true))) |
        | Band-cut selected mix | $(Child("bandcut_selected_mix", Slider(1:1:20; default=1, show_value=true))) |
        | Smooth mix trials | $(Child("smooth_mix_trials", Slider(10:10:2000; default=256, show_value=true))) |
        | Smooth mix active atoms | $(Child("smooth_mix_active_atoms", Slider(1:1:100; default=100, show_value=true))) |
        | Smooth mix control knots | $(Child("smooth_mix_control_knots", Slider(2:1:12; default=5, show_value=true))) |
        | Smooth mix minimum patch periods | $(Child("smooth_mix_min_patch_periods", Slider(1:1:20; default=5, show_value=true))) |
        | Smooth mix maximum patches | $(Child("smooth_mix_max_patches", Slider(0:1:20; default=8, show_value=true))) |
        | Smooth mix random seed | $(Child("smooth_mix_random_seed", NumberField(1:1:999999999; default=20260528))) |
        | Smooth mix top mixes | $(Child("smooth_mix_top_mixes", Slider(1:1:20; default=5, show_value=true))) |
        | Smooth mix selected rank | $(Child("smooth_mix_selected_rank", Slider(1:1:20; default=1, show_value=true))) |
        | Superatom output | $(Child("super_output", Select(["Direct DP path", "Reconstructed MFT"]; default="Direct DP path"))) |
        | Bandwise low-U weight | $(Child("low_u_weight", Slider(0.0:0.05:1.0; default=1.0, show_value=true))) |
        | Super atom max U jump | $(Child("max_jump", Slider(0.02:0.01:0.25; default=0.15, show_value=true))) |
        | Super atom period refinement | $(Child("period_refinement", Slider(1:1:8; default=1, show_value=true))) |
        | Super atom MFT bandwidth | $(Child("bandwidth_factor", NumberField(0.01:0.01:1.00; default=round(bandwidth_factor; digits=2)))) |
        | Final superatom peaks shown | $(Child("max_peaks", Slider(1:1:4; default=4, show_value=true))) |
        | Overlay | $(Child("overlay", Select(_global_uc_codebook_spec_options; default=0))) |
        """
    end
end

# ╔═╡ bb000011-3337-0000-0000-000000000001
global_uc_selected_codebook_spec = let
    idx = _global_uc_overlay_index(_global_uc_overlay_controls.overlay,
        global_uc_codebook_overlay_specs)
    idx <= 0 || idx > length(global_uc_codebook_overlay_specs) ?
        nothing : global_uc_codebook_overlay_specs[idx]
end;

# ╔═╡ bb000011-3338-0000-0000-000000000001
begin
    ui_global_uc_overlay_index = _global_uc_overlay_index(_global_uc_overlay_controls.overlay,
        global_uc_codebook_overlay_specs)
    ui_global_uc_super_mode = String(_global_uc_overlay_controls.super_mode)
    ui_global_uc_super_family = String(_global_uc_overlay_controls.family)
    ui_global_uc_super_threshold = Float64(_global_uc_overlay_controls.threshold)
    ui_global_uc_super_mix_size = Int(_global_uc_overlay_controls.mix_size)
    ui_global_uc_super_candidate_peaks = Int(_global_uc_overlay_controls.candidate_peaks)
    ui_global_uc_bandcut_top_mixes = Int(_global_uc_overlay_controls.bandcut_top_mixes)
    ui_global_uc_bandcut_selected_mix = Int(_global_uc_overlay_controls.bandcut_selected_mix)
    ui_global_uc_smooth_mix_trials = Int(_global_uc_overlay_controls.smooth_mix_trials)
    ui_global_uc_smooth_mix_active_atoms = Int(_global_uc_overlay_controls.smooth_mix_active_atoms)
    ui_global_uc_smooth_mix_control_knots = Int(_global_uc_overlay_controls.smooth_mix_control_knots)
    ui_global_uc_smooth_mix_min_patch_periods = Int(_global_uc_overlay_controls.smooth_mix_min_patch_periods)
    ui_global_uc_smooth_mix_max_patches = Int(_global_uc_overlay_controls.smooth_mix_max_patches)
    ui_global_uc_smooth_mix_random_seed = Int(_global_uc_overlay_controls.smooth_mix_random_seed)
    ui_global_uc_smooth_mix_top_mixes = Int(_global_uc_overlay_controls.smooth_mix_top_mixes)
    ui_global_uc_smooth_mix_selected_rank = Int(_global_uc_overlay_controls.smooth_mix_selected_rank)
    ui_global_uc_super_output = String(_global_uc_overlay_controls.super_output)
    ui_global_uc_super_low_u_weight = Float64(_global_uc_overlay_controls.low_u_weight)
    ui_global_uc_super_max_jump = Float64(_global_uc_overlay_controls.max_jump)
    ui_global_uc_super_period_refinement = Int(_global_uc_overlay_controls.period_refinement)
    ui_global_uc_super_bandwidth_factor = Float64(_global_uc_overlay_controls.bandwidth_factor)
    ui_global_uc_super_max_peaks = Int(_global_uc_overlay_controls.max_peaks)
    ui_global_uc_overlay_is_super = ui_global_uc_overlay_index == 0
    global_uc_selected_codebook_item = isnothing(global_uc_selected_codebook_spec) ?
        nothing : global_uc_selected_codebook_spec.item
    ui_global_uc_codebook_mode = isnothing(global_uc_selected_codebook_spec) ?
        ui_global_uc_super_family : global_uc_selected_codebook_spec.mode
    ui_global_uc_codebook_seed = isnothing(global_uc_selected_codebook_spec) ?
        missing : global_uc_selected_codebook_spec.seed
    ui_global_uc_codebook_wave_index = isnothing(global_uc_selected_codebook_spec) ?
        0 : global_uc_selected_codebook_spec.wave_index
    _global_uc_wave_labels = isnothing(global_uc_selected_codebook_spec) ?
        String[] : [global_uc_selected_codebook_spec.wave_label]
    ui_global_uc_overlay_label = ui_global_uc_overlay_is_super || isnothing(global_uc_selected_codebook_spec) ?
        "$(ui_global_uc_super_mode) ($(ui_global_uc_super_family))" : global_uc_selected_codebook_spec.display
end
...
global_uc_selected_codebook_payload = let
    ...
    if ui_global_uc_overlay_is_super
        ...
        if isempty(columns) || isempty(periods)
            ...
            cached = get!(global_uc_superatom_mft_cache, cache_key) do
                ...
                if isnothing(bank0)
                    ...
                    (; bank=bank0,
                     results=mft.perform_mft_analysis_batch!(bank0, W, dist; compute_phase=true))
                end
            end
            bank = cached.bank
        ...
        if isempty(waves) || wave_index > size(waves, 2)
            ...
            if isnothing(bank)
                ...
                res = only(mft.perform_mft_analysis_batch!(bank, W, dist; compute_phase=true))
...
if @isdefined(selected_plot_pair)
    WideCell(_plot_global_average_uc_consistency(
        ...
        codebook_rows=global_uc_selected_codebook_rows,
        velocity_range=velocity_range,
        title="Global Average Canonical-Pick U-c Consistency"))
else
    md""
end
...
let
    if ui_global_uc_overlay_is_super && !isnothing(global_uc_bandwise_superatom_summary)
...
if isempty(selected_mft_waveform_overlay_options)
    md"No $(ui_mft_mode) MFT waveform overlays are available for $(selected_plot_pair)."
else
    @bind selected_mft_waveform_overlay Select(selected_mft_waveform_overlay_options)
end
...
selected_marginal_combo_overlay_specs = let
    ...
    for (iitem, item) in enumerate(items)
        ...
        for k1 in 1:K1, k2 in 1:K2
            ...
            push!(specs, (; display, item_index=iitem, item, k1, k2,
                median_relative_error=Inf, max_relative_error=Inf, n_valid=0))
            push!(analysis_specs, (; pair_label, label, causal, acausal,
                distance=Float64(item.distance)))
        end
    end
    if !isempty(analysis_specs)
        analyses = _analyze_pair_branch_specs(analysis_specs; compute_phase=true)
        if haskey(analyses, selected_plot_pair)
            ...
            for (ispec, spec0) in enumerate(specs)
                if ispec <= length(batch.state_results)
                    ...
                    errs = Float64[r.relative_error for r in rows
                        if isfinite(r.relative_error) && r.relative_error >= 0.0]
                    mederr = isempty(errs) ? Inf : median(errs)
                    maxerr = isempty(errs) ? Inf : maximum(errs)
                    push!(scored, merge(spec0, (;
                        median_relative_error=mederr,
            ...
            sort!(scored, by=s -> (s.median_relative_error, s.max_relative_error,
                -s.n_valid, s.display))
            scored
        else
            specs
...
if isempty(selected_marginal_combo_overlay_specs)
    md"No K1/K2 marginal-combination overlays are available for $(selected_plot_pair)."
else
    @bind selected_marginal_combo_overlay Select(
        ["None"; String["$(s.display) | med err=$(isfinite(s.median_relative_error) ? round(s.median_relative_error; digits=3) : "NA")"
...
manifest_format = "2.0"
project_hash = "320bb2b08451763d2fbf3d57f6a3b476717b1658"
