...
# ╔═╡ ad6d425a-f694-4ea3-8de2-4247ea66483f


begin
    struct BroadcastCoherentEncCategoricalOneHot{T1,T2,T3,T4}
        chain::T1
        μ::T2
        loginvvar::T3
        λlogits::T4
    end
    Flux.@layer BroadcastCoherentEncCategoricalOneHot trainable = (chain, μ, loginvvar, λlogits)
    
    function (m::BroadcastCoherentEncCategoricalOneHot)(x)
        # First pass: get shared features and category logits
        X = m.chain(x)
        λlogits = m.λlogits(x)  # Shape: (K, ntau, batch)
        
        # For each category, compute conditional μ and loginvvar
        K = size(λlogits, 1)

		 # # For each category, compute q(z|x,c)
		μs = map(1:K) do c
			ohc = xpu(Float32.(Flux.onehotbatch(fill(c, size(X, 2), size(X, 3)), 1:K)))
			# Xc = vcat(X, ohc)
			Xc = X
			m.μ(Xc)
		end

		loginvvars = map(1:K) do c
			ohc = xpu(Float32.(Flux.onehotbatch(fill(c, size(x, 2), size(x, 3)), 1:K)))
			# Xc = vcat(X, ohc)
			Xc = X
			m.loginvvar(Xc)
        ...
        # Stack all category-specific parameters: shape (p, K, ntau, batch)
        μ = cat(μs..., dims=1)
        loginvvar = cat(loginvvars..., dims=1)
...
function filter_mode(d::AbstractMatrix, model, kopt; p=0.9)
    ...
    dnew, indices = if (:λlogits in keys(coherent_code))
        ...
        Ic = findall(x -> x[kopt] >= p, eachcol(λp))
        d[:, Ic], Ic
    else
...
begin
    ...
    function SymVAE(NN, k=1)
        sencb = if (k > 1)
            BroadcastCoherentEncCategoricalOneHot(NN.senc, NN.senc_μ, NN.senc_loginvvar, NN.senc_λlogits)
        else
...
function get_symae(t::DenseAE, nt, p, q, k)
	...
	P = p * 4
    # instance means
    senc_μ = Chain(Dense(P, p, activation), Dense(p, p)) |> xpu

    # inverse variances
    senc_loginvvar = Chain(Dense(P, p, activation), Dense(p, p)) |> xpu
	
   senc_λlogits = xpu(Chain(Dense(nt, nt, activation), Dense(nt, div(nt, 2), activation), Dense(div(nt, 2), k))) # only used when k>1
...
function get_symae(t::ConvAE, nt, p, q, k)
	...
	P =  k > 1 ? Pout + k : Pout
    senc_μ = xpu(Chain(Dense(P, p, activation), Dense(p, p)))
    senc_loginvvar = xpu(Chain(Dense(P, p, activation), Dense(p, p)))
