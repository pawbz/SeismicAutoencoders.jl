...
# ╔═╡ 7161f506-45f8-11f1-a131-0ddcfa4aa717
begin
	using Lux, Optimisers, Printf, Random, Statistics, Enzyme
	import Flux
...
# ╔═╡ 78abf4ab-0a43-4090-8ace-96263f7aa1d4
using Zygote

# ╔═╡ 7be28f20-e4d7-4bed-8a94-e69a28f04934
using CUDA

# ╔═╡ 316133e7-525f-4d52-a6f1-80b941da5038
begin
	XLA_FLAGS = get(ENV, "XLA_FLAGS", "")
	ENV["XLA_FLAGS"] = "$(XLA_FLAGS) --xla_gpu_enable_cublaslt=true"
end

# ╔═╡ 6b3de1b0-2ebb-414d-b557-e0e65ba85843
function make_dummy_classification_data(::Type{T}, nsamples::Int; rng, nclasses::Int=10) where {T}
    x = randn(rng, T, 32, 32, 3, nsamples)
    labels = rand(rng, 0:(nclasses - 1), nsamples)
    y = onehotbatch(labels, 0:(nclasses - 1))
    return x, y
end

# ╔═╡ da0a28ae-fbbb-40e2-9f3d-cd808e3c09e7
...
# ╔═╡ 1d4019f1-88ff-420b-83c7-a2f24e7a94e8
function SimpleCNN()
    return Chain(
        Chain(
...
# ╔═╡ c5f0b81b-b839-4745-a2f7-b2b6c7d6b97a
function FluxSimpleCNN()
    return Flux.Chain(
        Flux.Chain(
            Flux.Conv((3, 3), 3 => 16, Flux.relu; stride=2, pad=1),
            Flux.BatchNorm(16),
            Flux.Conv((3, 3), 16 => 32, Flux.relu; stride=2, pad=1),
            Flux.BatchNorm(32),
            Flux.Conv((3, 3), 32 => 64, Flux.relu; stride=2, pad=1),
            Flux.BatchNorm(64),
            Flux.Conv((3, 3), 64 => 128, Flux.relu; stride=2, pad=1),
            Flux.BatchNorm(128),
        ),
        x -> dropdims(mean(x; dims=(1, 2)); dims=(1, 2)),
        Flux.Chain(Flux.Dense(128 => 64, Flux.relu), Flux.BatchNorm(64), Flux.Dense(64 => 10)),
    )
end

# ╔═╡ 54f3b130-45fc-11f1-8314-d1cfd5507be5
function get_dummy_dataloaders(::Type{T}, batchsize;
    rng,
    ntrain::Int=8192,
    ntest::Int=2048,
    kwargs...) where {T}
    x_train, y_train = make_dummy_classification_data(T, ntrain; rng)
    x_test, y_test = make_dummy_classification_data(T, ntest; rng)
    trainloader = DataLoader((x_train, y_train); batchsize, shuffle=true, kwargs...)
    testloader = DataLoader((x_test, y_test); batchsize, shuffle=false, kwargs...)
    return trainloader, testloader
end

# ╔═╡ fca5a74b-87d1-45cb-9dff-a6dc8dd3e2f6
function flux_device(use_gpu::Bool)
    !use_gpu && return identity
    try
        Flux.gpu(rand(Float32, 1))
        return Flux.gpu
    catch err
        @printf "[Warn] Flux GPU unavailable, falling back to CPU: %s\n" sprint(showerror, err)
        return identity
    end
end

# ╔═╡ 00af2907-90c5-4c0a-b569-3131a9a85950
function flux_accuracy(model, dataloader, dev)
    total_correct, total = 0, 0
    for (x, y) in dataloader
        logits = model(dev(x))
        target_class = onecold(y)
        predicted_class = onecold(Flux.cpu(logits))
        total_correct += sum(target_class .== predicted_class)
        total += length(target_class)
    end
    return total_correct / total
...
# ╔═╡ e785ae73-7954-4a8d-8a2f-f86572dba532
function train_flux_model(;
    ...
    dev = flux_device(use_gpu)
    trainloader, testloader =
        get_dummy_dataloaders(Float32, batchsize; rng, partial=false)

    model = FluxSimpleCNN() |> dev
    opt_state = Flux.setup(Flux.Adam(lr), model)

    @printf "[Info] Training Flux/Zygote model on %s\n" use_gpu ? "GPU" : "CPU"
    @printf "%8s %18s %18s %18s %12s\n" "Epoch" "Loss" "Train Accuracy" "Test Accuracy" "Time (s)"
    ...
    for epoch in 1:epochs
        ...
        for (x, y) in trainloader
            ...
            loss, grads = Flux.withgradient(model) do m
                Flux.Losses.logitcrossentropy(m(xb), yb)
            end
        ...
        ttime = time() - stime
        push!(epoch_times, ttime)

        train_acc = flux_accuracy(model, trainloader, dev) * 100
        test_acc = flux_accuracy(model, testloader, dev) * 100
        @printf "%8d %18.6f %18.6f %18.6f %12.6f\n" epoch last_loss train_acc test_acc ttime
    end

    return (; backend="Flux/Zygote", epoch_times, last_loss, train_acc, test_acc)
end

# ╔═╡ c234ac91-41a6-4865-9cac-f3daf05f4fdd
dev = reactant_device(; force=true, device=1)

# ╔═╡ 7ddbf922-2ffc-4925-9e7c-731fcb1ac40e
function train_model(
    ...
    trainloader, testloader =
        get_dummy_dataloaders(prec_jl, batchsize; rng, partial=false) |> dev
    ...
    for epoch in 1:epochs
        ...
        for (i, (x, y)) in enumerate(trainloader)
            ...
            (_, loss, _, train_state) = Training.single_train_step!(
                Zygote(), loss_fn, (x, y), train_state; return_gradients=Val(false)
            )
        ...
        @printf "%8d %14.6f %18.6f %18.6f %12.6f\n" epoch lr train_acc test_acc ttime
    end

    return (; backend="Lux/Reactant/Enzyme", epoch_times=Float64[], last_loss=NaN32)
end

# ╔═╡ 7b89c8f8-f7c5-4719-999e-2e50b4814866
function main(;
    batchsize::Int=512,
...
# ╔═╡ 4715851e-5c3e-4f6a-8bdf-19211cc45b8e
lux_result = main(; epochs=5, batchsize=512)

# ╔═╡ 59e004f6-7b9f-4580-bd19-e4dfaf6d792f
 CUDA.device!(1)

# ╔═╡ 7b00a4ec-2370-4a3e-92d6-49e207070a8c
flux_result = train_flux_model(; epochs=3, batchsize=512, use_gpu=true)

# ╔═╡ 0fa07ac9-4591-4673-97d7-15d649e9191b
begin
    flux_mean_time = isempty(flux_result.epoch_times) ? NaN : mean(flux_result.epoch_times)
    @printf "\n%24s %18s %18s %18s\n" "Backend" "Mean Epoch (s)" "Train Accuracy" "Test Accuracy"
    @printf "%24s %18s %18s %18s\n" lux_result.backend "see epoch log" "see epoch log" "see epoch log"
    @printf "%24s %18.6f %18.6f %18.6f\n" flux_result.backend flux_mean_time flux_result.train_acc flux_result.test_acc
end
...
manifest_format = "2.0"
project_hash = "78d1249e83ecbb1f156ce63854449008aa67879c"
