...
Base.@kwdef struct VQVAE_Para
    ...
    # Optional receiver-geometry conditioning. `geometry_features` is a
    # standardized (geometry_dim, num_pairs) matrix selected by pair id.
    use_geometry_conditioning::Bool = false
    use_pair_id_conditioning::Bool = false
    geometry_dim::Int = 0
    condition_dim::Int = 16
    geometry_features::Union{Nothing,Matrix{Float32}} = nothing
end
...
begin
    ...
    function single_pair_id(ids::AbstractVector{<:Integer})
        isempty(ids) && return nothing
        pid = ids[1]
        @inbounds for i in 2:length(ids)
            ids[i] == pid || return nothing
        end
        return pid
    end

    function pair_id_chunks(ids::AbstractVector{<:Integer})
    ...
    function (vq::PairSpecificMultiscaleRVQQuantizer)(z_e::AbstractMatrix{Float32}, pair_ids;
        ...
        pid_single = Zygote.@ignore single_pair_id(ids)
        if pid_single !== nothing
            rt_single = vq.coarse[pid_single](z_coarse_in; beta_commit=beta_commit, training=training)
            z_q_coarse = rt_single.z_q
            coarse_indices = training ? nothing : Int.(cpu(rt_single.indices))
            vq_loss_small = rt_single.vq_loss
            commit_small = rt_single.commit_loss
            entropy_small = rt_single.entropy_loss
            perplexity_small = rt_single.perplexity
            num_active_pairs = 1
        else
            z_q_coarse = zero(z_e)
            coarse_indices = training ? nothing : zeros(Int, N)
            vq_loss_small = 0f0
            commit_small = 0f0
            entropy_small = 0f0
            perplexity_small = 0f0
            num_active_pairs = 0
            for (pid, chunk) in Zygote.@ignore(pair_id_chunks(ids))
                rt_small = vq.coarse[pid](z_coarse_in[:, chunk]; beta_commit=beta_commit, training=training)
                if training
                    sel = collect(chunk)
                    sel_mask = xpu(Float32.(reshape(collect(1:N), N, 1) .== reshape(sel, 1, :)))
                    z_q_coarse = z_q_coarse .+ rt_small.z_q * sel_mask'
                else
                    z_q_coarse[:, chunk] .= rt_small.z_q
                    coarse_indices[chunk] .= Int.(cpu(rt_small.indices))
                end
...
begin
    struct VQVAE{E,M,P,VQ,D,C,G,B}
        ...
        geometry_conditioner::C
        geometry_features::G
        pair_condition_embedding::B
        d::Int             # codebook embedding dimension
        latent_len::Int    # encoder latent-time length L
        condition_dim::Int
        condition_mode::Symbol
        num_pairs::Int
    end
    Flux.@layer VQVAE trainable = (encoder, metric_proj, pre_vq, quantizer, decoder,
        geometry_conditioner, pair_condition_embedding)

    codebook_size(m) = m.quantizer.K

    function pair_condition(m, pair_ids_cpu, n_expected::Int)
        m.condition_dim == 0 && return nothing
        length(pair_ids_cpu) == n_expected || error("Expected $n_expected pair ids, got $(length(pair_ids_cpu))")
        if m.condition_mode === :geometry
            isnothing(m.geometry_features) && error("Geometry conditioning is enabled, but geometry_features is nothing.")
            cond_in = xpu(m.geometry_features[:, pair_ids_cpu])
            cond = m.geometry_conditioner(cond_in)
            size(cond, 1) == m.condition_dim ||
                error("Geometry conditioner returned $(size(cond, 1)) rows, expected $(m.condition_dim)")
            return cond
        elseif m.condition_mode === :pair_id
            isnothing(m.pair_condition_embedding) && error("Pair-id conditioning is enabled, but pair_condition_embedding is nothing.")
            all(1 .<= pair_ids_cpu .<= m.num_pairs) || error("pair ids must lie in 1:$(m.num_pairs)")
            pair_onehot = xpu(Float32.(Array(Flux.onehotbatch(pair_ids_cpu, 1:m.num_pairs))))
            return m.pair_condition_embedding * pair_onehot
        else
            error("Unsupported condition_mode=$(m.condition_mode).")
        end
    end

    function metric_latents(m, feat_map, pair_ids_cpu=nothing)
        ...
        cond = isnothing(pair_ids_cpu) ? nothing : pair_condition(m, pair_ids_cpu, B)
        pre_vq_input = isnothing(cond) ? z_metric_flat : vcat(z_metric_flat, cond)
        z_pre = m.pre_vq(pre_vq_input)
        ...
        return z_metric, z_metric_flat, z_pre, cond
    end
    ...
    function encode(m, x, pair_ids; beta_commit::Float32=0.25f0, training::Bool=false)
        ...
        pair_ids_cpu = Zygote.@ignore validate_pair_ids(pair_ids,
            ...
            N_total)
        z_metric, z_metric_flat, z_e, condition = metric_latents(m, feat_map, pair_ids_cpu)
...
function rebuild_latent_index!(idx::LatentIndex, model, X, pair_ids; Mnn::Int=idx.Mnn, batch_size::Int=256)
    ...
    idx.embeddings = zeros(Float32, 0, 0)
...
function contiguous_pair_index_map(pair_ids)
    ids = Int.(pair_ids_to_cpu(pair_ids))
    mapping = Dict{Int,UnitRange{Int}}()
    pair_order = Int[]
    i = 1
    n = length(ids)
    while i <= n
        ...
        haskey(mapping, pid) && return nothing
        mapping[pid] = start_i:(i - 1)
        push!(pair_order, pid)
    end
    return (; mapping, pair_order)
end
...
function build_pairwise_batches(data, para, training_para; shuffle_pairs::Bool=true)
    ...
    contiguous = contiguous_pair_index_map(pair_ids)
    if isnothing(contiguous)
        pair_map = pair_id_index_map(pair_ids)
        pair_list = sort!(collect(keys(pair_map)))
    else
        pair_map = contiguous.mapping
        pair_list = contiguous.pair_order
    end
    ...
    for start_pair in 1:pairs_per_batch:length(ordered_pairs)
        active_pairs = ordered_pairs[start_pair:min(start_pair + pairs_per_batch - 1, length(ordered_pairs))]
        batch_indices = Int[]
...
function get_vqvae(para)
    ...
    if para.use_geometry_conditioning && para.use_pair_id_conditioning
        error("Choose only one conditioning mode: geometry or pair_id, not both.")
    end
    condition_mode = para.use_geometry_conditioning ? :geometry :
        (para.use_pair_id_conditioning ? :pair_id : :none)
    condition_dim = condition_mode === :none ? 0 : para.condition_dim
    if para.use_pair_id_conditioning
        para.num_pairs >= 1 || error("num_pairs must be >= 1 when pair-id conditioning is enabled.")
        para.condition_dim > 0 || error("condition_dim must be > 0 when pair-id conditioning is enabled.")
    end
    if para.use_geometry_conditioning
        para.geometry_dim > 0 || error("geometry_dim must be > 0 when geometry conditioning is enabled.")
        para.condition_dim > 0 || error("condition_dim must be > 0 when geometry conditioning is enabled.")
        isnothing(para.geometry_features) && error("geometry_features must be provided when geometry conditioning is enabled.")
        size(para.geometry_features, 1) == para.geometry_dim ||
            error("geometry_features has $(size(para.geometry_features, 1)) rows, expected geometry_dim=$(para.geometry_dim).")
        size(para.geometry_features, 2) == para.num_pairs ||
            error("geometry_features has $(size(para.geometry_features, 2)) columns, expected num_pairs=$(para.num_pairs).")
    end
    geometry_conditioner = para.use_geometry_conditioning ?
        Chain(Dense(para.geometry_dim, para.condition_dim, activation),
              Dense(para.condition_dim, para.condition_dim, activation)) |> xpu :
        nothing
    geometry_features = para.use_geometry_conditioning ? Float32.(para.geometry_features) : nothing
    pair_condition_embedding = para.use_pair_id_conditioning ?
        xpu(randn(Float32, para.condition_dim, para.num_pairs) .* 0.01f0) :
        nothing
    pre_vq = Dense(latent_len * para.d + condition_dim, para.d) |> xpu
    ...
    metric_dim = para.d * latent_len + condition_dim
    @info "VQ-VAE geometry" nt = para.nt latent_len metric_dim condition_mode condition_dim enc_outsize dec_upstrides dec_out_len latent_time_start latent_time_end

    model = VQVAE(xpu(encoder), xpu(metric_proj), xpu(pre_vq), quantizer, xpu(decoder),
        geometry_conditioner, geometry_features, pair_condition_embedding,
        para.d, latent_len, condition_dim, condition_mode, para.num_pairs)
